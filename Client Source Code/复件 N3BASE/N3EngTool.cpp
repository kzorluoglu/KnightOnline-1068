...
void CN3EngTool::RenderGrid(float fScale)
{
	...
	stm.Scale(fScale, fScale, fScale); // ������ ����
...
void CN3EngTool::RenderTexturePreview(CN3Texture *pTex, HWND hWndDiffuse)
{
	...
	if(hWndDiffuse != NULL)
	{
		...
		if(pTex->Get() != NULL)
		{
			...
			if(fRatioDest > 1.0f) // ���ΰ� ����
			{
				int nDec = (nW - nH) / 2;
				rcTex.left += nDec;
				rcTex.right -= nDec;
			}
			else if(fRatioDest < 1.0f) // ���ΰ� ����
			{
				int nDec = (nH - nW) / 2;
				rcTex.top += nDec;
				rcTex.bottom -= nDec;
...
//			m_DeviceInfo.pModes[i].Height == dwHeight && 
		if(	m_DeviceInfo.pModes[i].Format == dm.Format) // ��尡 ��ġ�ϸ�
		{
			this->FindDepthStencilFormat(0, m_DeviceInfo.DevType, m_DeviceInfo.pModes[i].Format, &DevParam.AutoDepthStencilFormat); // ���̿� ���ٽ� ���۸� ã�´�.
			break;
		}
	}
	...
	if(rval != D3D_OK)
	{
		...
		if(rval != D3D_OK)
		{
			...
			MessageBox(s_hWndBase, szDebug, "Direct3D Device ���� ����", MB_OK);
			this->Release();
			return false;
		}
#if _DEBUG
		OutputDebugString("HardWare TnL �� �������� �ʽ��ϴ�. SoftWare TnL �� ���� �մϴ�.\n");
#endif
	}
	...
	m_lpD3DDevExtra->SetRenderState(D3DRS_ZENABLE, D3DZB_TRUE); // Z���� ��밡��
...
//    m_lpD3DDevExtra->SetRenderState( D3DRS_AMBIENT,        0x00444444 );
	...
	m_lpD3DDevExtra->SetRenderState(D3DRS_ALPHAFUNC, D3DCMP_GREATER); // �⺻ ���� ��� - �����ָ� ���� �ؽ�ó���� ���ٳ��⵵ �Ѵ�.
