...
void CN3Camera::Release()
{
	...
	m_Data.fFOV = D3DXToRadian(72.0f); // �⺻�� 72 ��
	m_Data.fNP = 0.3f;
	m_Data.fFP = 256.0f;
...
void CN3Camera::LookAround(float fRadianX, float fRadianY)		//At Postion�� �߽����� ī�޶� ����..���� ��ġ�� �ٲ��..
{
...
#endif
*/
	m_vPos = m_vAt + v1; // Rotation �� LookAt Position ó��, Scale �� UpVector ó�� ����..
}
...
void CN3Camera::Rotate(float fRadianX, float fRadianY)
{
	...
	m_v1 = m_vAt - m_vPos; // Rotation �� LookAt Position ó��, Scale �� UpVector ó�� ����..
	m_v1 *= mtx;
	m_vScale *= mtx;
...
{
	__Vector3 vDir = m_vAt - m_vPos; // Rotation �� LookAt Position ó��, Scale �� UpVector ó�� ����..
	vDir.Normalize();
	...
	m_vPos += vMove;
	m_vAt += vMove; // Rotation �� LookAt Position ó��, Scale �� UpVector ó�� ����..
}
...
//	if(dwZ) s_lpD3DDev->SetRenderState(D3DRS_ZENABLE, FALSE);
	...
	__Material mtl; mtl.Init(); // ���..
	s_lpD3DDev->SetMaterial(&mtl);
	s_lpD3DDev->SetTexture(0, NULL);

	s_lpD3DDev->SetFVF(FVF_XYZCOLOR);
	s_lpD3DDev->DrawIndexedPrimitiveUP(D3DPT_LINELIST, 0, 24, 12, wIndices, D3DFMT_INDEX16, vFrustums, sizeof(__VertexColor)); // ���׸���..
...
void CN3Camera::Tick(float fFrm)
{
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	// 	__Vector3 m_vEye, m_vUp ->> m_vPos, m_vScale �� ����Ѵ�.. �߿�!!
	m_Data.vEye = m_vPos;
	m_Data.vAt  = m_vAt;
	m_Data.vUp  = m_vScale; // Up Vector ó�� ����.
	/////////////////////////////////////////////////////////////////////////////////////////////////////////

	::D3DXMatrixLookAtLH(&m_Data.mtxView, &m_Data.vEye, &m_Data.vAt, &m_Data.vUp); // Look At ����
	::D3DXMatrixInverse(&m_Data.mtxViewInverse, NULL, &m_Data.mtxView); // View Inverse ��� ���ϱ�..
	CN3Base::s_lpD3DDev->GetViewport(&m_Data.vp); // View port ��������...
	m_Data.fAspect = (float)m_Data.vp.Width / (float)m_Data.vp.Height; // ��Ⱦ��
...
//							__Vector3(fPL * -fS, fPL * -fS / m_Data.fAspect, fPL) }; // LeftBottom
	// Frustom �� Front, Left, Top, Right, Bottom �� ������ 4���� ���� ���� ���..
	...
	*/
	...
	{
		vEyeTmp = m_vPos;
		vEyeTmp -= (m_Data.vFrustumNormals[0]) * (i * 3.0f); // ī�޶� ��ġ�� 10.0 m �ڷ� ����. -> Shape Ŭ���ο� ����..
