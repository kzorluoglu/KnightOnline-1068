...
void CN3Joint::Render(const __Matrix44* pMtxParent, float fUnitSize)
{
	...
	if(m_pParent) // �θ� ������ �̾��ִ� ��..
	{
		...
		s_lpD3DDev->DrawPrimitiveUP(D3DPT_LINELIST, 1, vBone, sizeof(__VertexColor)); // ���׸���..
	}
	...
	s_lpD3DDev->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 12, vBoxes, sizeof(__VertexColor)); // �ڽ� �׸���..
	s_lpD3DDev->SetTransform(D3DTS_WORLD, &mtxAxis);
	s_lpD3DDev->DrawPrimitiveUP(D3DPT_LINELIST, 3, vAxis, sizeof(__VertexColor)); // �� �׸���..
...
BOOL CN3Joint::FindPointerByName(const char* szName, CN3Joint *&pJoint) // �̸��� ������ �ش� ����� �����͸� �����ش�..
{
...
//	���� ��� ��� �ڵ�...
//	m_Matrix.Identity();
//	mtx.Rotation(m_vRot.x, m_vRot.y, m_vRot.z); m_Matrix *= mtx; // �θ� �� ��ŭ ȸ��.
//	mtx.Scale(m_vScale); m_Matrix *= mtx; // �θ� �� ��ŭ ȸ��.
//	mtx.Rotation(m_vOrigin.x, m_vOrigin.y, m_vOrigin.z); m_Matrix *= mtx; // �θ� ���� �� ��ŭ ȸ��.
//	mtx.Identity(); mtx.PosSet(m_vPos); m_Matrix *= mtx; // �̵�

//	if(m_RotSeq == ROT_SEQ_XYZ)
//	{
//		m_Matrix.Rotation(m_vRot.x, m_vRot.y, m_vRot.z); // �θ� �� ��ŭ ȸ��. // XYZ ȸ���϶�..
//	}
//	else if(ROT_SEQ_YXZ)
//	{
//		m_Matrix.Identity();
//		mtx.RotationY(m_vRot.y); m_Matrix *= mtx; // YXZ ȸ���϶�
//		mtx.RotationX(m_vRot.x); m_Matrix *= mtx;
//		mtx.RotationZ(m_vRot.z); m_Matrix *= mtx;
//	}

	::D3DXMatrixRotationQuaternion(&m_Matrix, &m_qRot); // ȸ��.. ���ʹϾ� ���..
	
	m_Matrix._11 *= m_vScale.x;
	m_Matrix._22 *= m_vScale.y;
	m_Matrix._33 *= m_vScale.z; // ������

	if(m_bHaveOriginRot) m_Matrix *= m_MtxOriginRot; // �θ� ���� �� ��ŭ ȸ��.
	
	m_Matrix._41 += m_vPos.x;
	m_Matrix._42 += m_vPos.y;
	m_Matrix._43 += m_vPos.z; // �̵�...
		
	if(m_pParent) m_Matrix *= m_pParent->m_Matrix; // �θ� ���
}
...
//	__ASSERT(1.0f == (fWeight0 + fWeight1));
	...
	this->ReCalcMatrix(); // Matrix ���...
	memcpy(&(pMtxs[nJointIndex]), &m_Matrix, sizeof(__Matrix44)); // Matrix ����..

	nJointIndex++; // Joint Index ����..
...
void CN3Joint::KeyDelete(CN3Joint *pJoint, int nKS, int nKE)
{
	...
	for(i = 0; i < nCC; i++)
	{
		...
		pChild->KeyDelete(pChild, nKS, nKE); // ���� ����Ʈ�� ����..
	}
}
...
void CN3Joint::AddKey(CN3Joint *pJDest, CN3Joint *pJSrc, int nIndexS, int nIndexE)
{
	...
	for(; pPos0 && pPos1; pJSrc->m_ChildrenList.Next(pPos0), pJDest->m_ChildrenList.Next(pPos1))
	{
		...
		this->AddKey(pChildDest, pChildSrc, nIndexS, nIndexE); // ���ȣ��
	}
}
...
//	pJDest->m_RotSeq = pJSrc->m_RotSeq;
	...
	for(pPos = pJSrc->m_ChildrenList.Position(0); pPos; pJSrc->m_ChildrenList.Next(pPos))
	{
		...
		pJDest->ChildAdd(pChildDest); // �ڽ� �߰�..

		CN3Joint* pChildSrc = pJSrc->m_ChildrenList.Get(pPos);
		this->CopyExceptAnimationKey(pChildDest, pChildSrc); // ���ȣ��
	}
}
