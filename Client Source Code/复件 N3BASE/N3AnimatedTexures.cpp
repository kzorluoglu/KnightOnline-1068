...
bool CN3AnimatedTexures::Load(HANDLE hFile)
{
	...
	int nL = 0;
	char szFN[256] = "";
	m_TexRefs.assign(iTC, NULL); // Texture Pointer Pointer �Ҵ�..
	for(int i = 0; i < iTC; i++) // Texture Count ��ŭ ���� �̸� �о �ؽ�ó �θ���..
	{
...
bool CN3AnimatedTexures::Save(HANDLE hFile)
{
	...
	for(int i = 0; i < iTC; i++) // Texture Count ��ŭ ���� �̸� �о �ؽ�ó �θ���..
	{
		...
		if(nL > 0)
		{
			WriteFile(hFile, m_TexRefs[i]->FileName().c_str(), nL, &dwRWC, NULL); // �ؽ�ó ���� �̸�..
		}
	}
