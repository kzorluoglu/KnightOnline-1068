...
void CN3VMesh::Release()
{
	...
	m_vCenter.Set(0,0,0); // Mesh Vertices �� �߰���..
	m_fRadius = 0.0f; // ������
}
...
bool CN3VMesh::Load(HANDLE hFile)
{
	...
	int nVC;
	ReadFile(hFile, &nVC, 4, &dwRWC, NULL); // ������ �б�..
	if(nVC > 0)
	{
		this->CreateVertices(nVC); // Vertex Buffer ���� �� ������ ä���
	...
	if(nIC > 0)
	{
		this->CreateIndex(nIC); // Vertex Buffer ���� �� ������ ä���
		ReadFile(hFile, m_pwIndices, nIC * 2, &dwRWC, NULL);
	}

	this->CalcRadiusAndCenter(); // �߽����� �������� ����� �ش�..
...
bool CN3VMesh::Save(HANDLE hFile)
{
	...
	WriteFile(hFile, &m_nVC, 4, &dwRWC, NULL); // ������ �б�..
	if(m_nVC > 0) 
	{
	...
	if(m_nIC > 0)
	{
		WriteFile(hFile, m_pwIndices, m_nIC * 2, &dwRWC, NULL); // Index Buffer ������ ����..
	}
...
void CN3VMesh::CreateVertices(int nVC)
{
	...
	memset(m_pVertices, 0, nVC * sizeof(__Vector3)); // Vertex Buffer ����
	m_nVC = nVC;
}
...
void CN3VMesh::CreateIndex(int nIC)
{
	...
	memset(m_pwIndices, 0, nIC * 2); // Index Buffer ����
	m_nIC = nIC;
}
...
bool CN3VMesh::CheckCollision(const __Matrix44* pMtxWorld, int xScreen, int yScreen)
{
	...
	if(m_nIC > 0)  // �ε����� �ִ� ���
	{
		...
		for(int i = 0; i < nFC; i++)
		{
			if(::IntersectTriangle(vPos, vDir, m_pVertices[m_pwIndices[i*3+0]], m_pVertices[m_pwIndices[i*3+1]], m_pVertices[m_pwIndices[i*3+2]])) // ù° ���Ͱ� ��ġ��..
			{
				return true;
			}
		}
	...
	else // �ε��� ���� ���� �ִ� ���
	{
		...
		for(int i = 0; i < nFC; i++)
		{
			if(::IntersectTriangle(vPos, vDir, m_pVertices[i*3+0], m_pVertices[i*3+1], m_pVertices[i*3+2])) // ù° ���Ͱ� ��ġ��..
			{
				return true;
			}
		}
...
bool CN3VMesh::CheckCollision(const __Matrix44* pMtxWorld, __Vector3& v0, __Vector3& v1, __Vector3 &vCollision, __Vector3 &vNormal)
{
	...
	if(m_nIC > 0) // �ε����� �ִ� ���
	{
		...
		for(int i = 0; i < nFC; i++)
		{
			...
			if(::IntersectTriangle(vPos0, vDir, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2])) // ù° ���Ͱ� ��ġ��..
			{
				if(false == ::IntersectTriangle(vPos1, vDir, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2])) // ��°�� �� ��ġ��..
				{
					// �浹�̴�..
					float fT, fU, fV;
	...
	else // �ε��� ���� ���� �ִ� ���
	{
		...
		for(int i = 0; i < nFC; i++)
		{
			...
			if(::IntersectTriangle(v0, vDir, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2])) // ù° ���Ͱ� ��ġ��..
			{
				if(false == ::IntersectTriangle(v1, vDir, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2])) // ��°�� �� ��ġ��..
				{
					// �浹�̴�..
					float fT, fU, fV;
