...
/*	m_vCollisionBoxes[0].Set(-0.3f, 1.8f, -0.15f); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[1].Set( 0.3f, 1.8f, -0.15f); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[2].Set( 0.3f, 0.0f, -0.15f); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[3].Set(-0.3f, 0.0f, -0.15f); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[4].Set( 0.3f, 1.8f,  0.15f); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[5].Set(-0.3f, 1.8f,  0.15f); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[6].Set(-0.3f, 0.0f,  0.15f); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[7].Set( 0.3f, 0.0f,  0.15f); // �浹 üũ�� ���� ������

	m_wCollisionIndices[0] = 0; m_wCollisionIndices[1] = 1; m_wCollisionIndices[2] = 2; // �浹 üũ�� ���� ������ �ε��� - ����
	m_wCollisionIndices[3] = 0; m_wCollisionIndices[4] = 2; m_wCollisionIndices[5] = 3; // �浹 üũ�� ���� ������ �ε���
	m_wCollisionIndices[6] = 1; m_wCollisionIndices[7] = 4; m_wCollisionIndices[8] = 7; // �浹 üũ�� ���� ������ �ε��� - ������
	m_wCollisionIndices[9] = 1; m_wCollisionIndices[10] = 2; m_wCollisionIndices[11] = 2; // �浹 üũ�� ���� ������ �ε��� 
	m_wCollisionIndices[12] = 5; m_wCollisionIndices[13] = 0; m_wCollisionIndices[14] = 3; // �浹 üũ�� ���� ������ �ε��� - ����
	m_wCollisionIndices[15] = 5; m_wCollisionIndices[16] = 3; m_wCollisionIndices[17] = 2; // �浹 üũ�� ���� ������ �ε���
	m_wCollisionIndices[18] = 4; m_wCollisionIndices[19] = 5; m_wCollisionIndices[20] = 6; // �浹 üũ�� ���� ������ �ε��� - ����
	m_wCollisionIndices[21] = 4; m_wCollisionIndices[22] = 6; m_wCollisionIndices[23] = 7; // �浹 üũ�� ���� ������ �ε���
	m_wCollisionIndices[24] = 5; m_wCollisionIndices[25] = 4; m_wCollisionIndices[26] = 1; // �浹 üũ�� ���� ������ �ε��� - ����
	m_wCollisionIndices[27] = 5; m_wCollisionIndices[28] = 1; m_wCollisionIndices[29] = 0; // �浹 üũ�� ���� ������ �ε���
	m_wCollisionIndices[30] = 3; m_wCollisionIndices[31] = 2; m_wCollisionIndices[32] = 7; // �浹 üũ�� ���� ������ �ε��� - �Ʒ���
	m_wCollisionIndices[33] = 4; m_wCollisionIndices[34] = 7; m_wCollisionIndices[35] = 6; // �浹 üũ�� ���� ������ �ε���
*/
	this->Release();
}
...
//	m_pSkinCollision = NULL;
	...
	m_fFrmCurUpper = 0; // ���� ������ 0 - ��ü
	m_fFrmCurLower = 0; // ���� ������ 1 - ��ü

	m_nAniCur = -1; // ���� ���ϸ��̼�
	m_nAniPrev = -1; // ���� ���ϸ��̼�..
	m_nAniLoopCountCur = 0; // ���ϸ��̼� ���� ī��Ʈ..

	m_fFrmBlendUpper = 0.0f; // �������� ���ϸ��̼� ������ ������..
	m_fFrmBlendLower = 0.0f; // �������� ���ϸ��̼� ������ ������..
	m_fBlendTimeCur = 0.0f; // ���� ������ �ð�..
	m_fBlendTime = 0.0f; // �������� �ð�. (�ʴ���)
...
bool CN3Chr::Load(HANDLE hFile)
{
	...
	this->RemakePlugTracePolygons(); // Plug Trace �� ���̴� ������ �ʱ�ȭ...
...
//	}
	...
	this->Init(); // ���� �� Vertex ���� �ʱ�ȭ �����ش�.. �׸��� �ִ� �ּҰ��� ã��... �浹 �ڽ� �޽ø� �ʱ�ȭ ��Ų��..
...
void CN3Chr::Tick(float fFrm)
{
	...
	{
		if(m_pAniCur && fFrm == FRAME_SELFPLAY) // ���� ������ ���ϸ��̼� ��� �ڵ� �÷���
		{
			bool bBlendTick = false;

			if(0.0f != m_fBlendTime) // Blending �� ���ϸ��̼��� ������..
			{
				// ���� �����ϴ� �κ�.. ������ ���� ������ ������.. �߰����ϼ��� ������ �������� ������..
				...
				if(m_fBlendTimeCur > m_fBlendTime) // Blending �� ������.
				{
					m_fBlendTime = 0.0f;
			...
			else
			{
				...
				if(m_fFrmCurUpper >= m_pAniCur->fFrmEndUpper)
				{
					...
					if(m_nAniLoopCountCur > 0) // Animation Loop Count �� �����Ǿ� ������..
					{
						m_nAniLoopCountCur--;
						if(m_nAniLoopCountCur <= 0) // ������ �� ��ŭ ������ ������..
						{
							float fFrmPrev = m_pAniCur->fFrmEndUpper;
							this->AniCurSet(m_nAniPrev, 0, 0.3f); // ���� ���ϸ��̼����� ������..
			...
			else
			{
				...
				int nJI = 0;
				m_pJointRef->MatricesGet(m_pMtxs, nJI); // Matrix ���..
				
				fFrm = m_fFrmCurUpper; // ���� �������� ����س��´�..
			}
		}
		else
		{
		...
		for(void* pPos = m_Plugs.Position(0); pPos; m_Plugs.Next(pPos))
		{
			...
			if(fFrm >= m_pAniCur->fFrmPlugTraceStart0 && fFrm <= m_pAniCur->fFrmPlugTraceEnd0) // ���� �������� �ȿ� ������..
			{
				m_bRenderPlugTraces[nPlugTrace] = true;
				if(m_fFrmPrev <= m_pAniCur->fFrmPlugTraceStart0) // �������� ����..
				{
...
void CN3Chr::Init()
{
	...
	if(nJC > 0)
	{
		...
		__Vector3 vTmp;
		for(int i = 0; i < nJC; i++) // ���� ���� ��ŭ �� ������ ���� �����Ϳ� ������� �����´�..
		{
			float fTmp = 1.0f;
			::D3DXMatrixInverse(&m_pMtxInverses[i], &fTmp, &m_pMtxs[i]); // ���� �࿡ ���� ������� ���Ѵ�..
		...
		m_fRadius = (m_vMax - m_vMin).Magnitude(); // ������ ���ϱ�..
	}
	else
	{
		m_vMin.Zero();
...
/*	m_vCollisionBoxes[0].Set(vMin.x, vMax.y, vMin.z); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[1].Set(vMax.x, vMax.y, vMin.z); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[2].Set(vMax.x, vMin.y, vMin.z); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[3].Set(vMin.x, vMin.y, vMin.z); // �浹 üũ�� ���� ������
	
	m_vCollisionBoxes[4].Set(vMax.x, vMax.y, vMax.z); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[5].Set(vMin.x, vMax.y, vMax.z); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[6].Set(vMin.x, vMin.y, vMax.z); // �浹 üũ�� ���� ������
	m_vCollisionBoxes[7].Set(vMax.x, vMin.y, vMax.z); // �浹 üũ�� ���� ������
*/
}
...
void CN3Chr::PartSet(int nPart, const char *szFN)
{
	...
	if(strcmp(szFN, pPart->Name()) == 0) return; // ���� �����͸� �Ѿ��..
...
void CN3Chr::PlugSet(int nPlug, const char *szFN)
{
	...
	if(strcmp(szFN, pPlug->Name()) == 0) return; // ���� �����͸� �Ѿ��..
...
void CN3Chr::RemakePlugTracePolygons()
{
	...
	for(void* pPos = m_Plugs.Position(0); pPos; m_Plugs.Next(pPos))
	{
		...
		for(int i = 0; i < pPlug->m_nTraceStep + 1; i++) // �ܰ������� ���� �����ش�..
		{
...
int CN3Chr::AniCurSet(int nAni, int nLoopCount, float fBlendTime)
{
	...
	if(nAni >= 0 && nAni < nCount)
	{
		...
		if(fBlendTime > 0)
		{
			m_fFrmBlendUpper = m_fFrmCurUpper; // Blending �� ���ϸ��̼� Frame..
		}
		else
		{
			m_fFrmBlendUpper = m_pAniCur->fFrmStartUpper; // Blending �� ���ϸ��̼� Frame..
		}
...
/*	// Compute the vector of the pick ray in screen space
	...
	for(int i = 0; i < 8; i++)
	{
		svColls[i] = m_vCollisionBoxes[i] * m_Matrix; // ���� �浹 �ڽ��� ���� ��ķ� �����ְ�..
	}
	...
	for(i = 0; i < 12; i++)
	{
		...
		if(::IntersectTriangle(vPos, vDir, svColls[nCI0], svColls[nCI1], svColls[nCI2])) // ù° ���Ͱ� ��ġ��..
		{
			return true;
		}
	}
...
/*	__Vector3 vDir = v1 - v0;
	...
	for(i = 0; i < 12; i++)
	{
		...
		if(::IntersectTriangle(v0, vDir, svColls[nCI0], svColls[nCI1], svColls[nCI2])) // ù° ���Ͱ� ��ġ��..
		{
			if(false == ::IntersectTriangle(v1, vDir, svColls[nCI0], svColls[nCI1], svColls[nCI2])) // ��°�� �� ��ġ��..
			{
				// �浹�̴�..
				float fT, fU, fV;
