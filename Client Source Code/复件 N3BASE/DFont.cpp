...
HRESULT CDFont::SetText(const char* pszText, DWORD dwFlags)
{
	...
	if (iStrLen>0)
	{
		...
		while(iCount<iStrLen)
		{
			...
			else if (pszText[iCount] & 0x80)	// 2BYTE ����
			{
		...
		SIZE size2;	// �ѱ� �ݱ����� ũ��..
		GetTextExtentPoint32( m_hDC, "��", lstrlen("��"), &size2 );
	...
	while(iCount<iStrLen)
	{
		if ('\n' == pszText[iCount])		// \n
		{
			...
			if (sx != x)
			{
				...
				assert(dwNumTriangles+2 < MAX_NUM_VERTICES);		// Vertex buffer�� ���ڶ���.
...
//				*pVertices++ = InitFont3DVertex( D3DXVECTOR3(vtx_sx+0,vtx_sy+0, 0), D3DXVECTOR3(0,0,-1), tx1, ty1 );
	...
	for (i=0; i<iCount; ++i)
	{
		TempVertices[i].p /= ((float)m_dwFontHeight);			// ���� ũ��� ���̱�
