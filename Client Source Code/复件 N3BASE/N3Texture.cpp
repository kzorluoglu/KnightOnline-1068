...
bool CN3Texture::LoadFromFile(const char* szFileName)
{
	...
	if(strstr(m_szFileName, ":") || strstr(m_szFileName, "\\\\") || strstr(m_szFileName, "//")) // ���ڿ��� ':', '\\', '//' �� ��� ������ ��ü ����̴�..
	{
	...
	if(lstrcmpi(&szFullPath[nFNL-3], "DXT") == 0)
	{
		...
		{
			char szErr[512];
			wsprintf(szErr, "%s - ������ ���ų� �б� ������� ������ ������ �źεǾ����ϴ�.", szFullPath);
...
bool CN3Texture::Load(HANDLE hFile)
{
	...
	ReadFile(hFile, &m_Header, sizeof(m_Header), &dwRWC, NULL); // ����� �д´�..
	...
	if(D3DFMT_DXT1 == fmtOld) // DXT Format �� �о�� �ϴµ� ������ �Ǵ��� �ȵǴ��� ���� �����ȵǸ� ��ü ������ ���Ѵ�.
	{
		...
		D3DFMT_DXT5 == fmtOld )
	{
		...
		if(TRUE == bDXTSupport)
		{
			for(int i = 0; i < nMipMapCount; i++)
			{
				...
				ReadFile(hFile, (BYTE*)LR.pBits, sd.Size, &dwRWC, NULL); // �Ϸķ� �� �����͸� ����..
			...
			{
				nSkip += nW * nH * 2; // DXT1 ������ 1/4 �� ����..
			}
			::SetFilePointer(hFile, nSkip, 0, FILE_CURRENT); // �ǳʶٰ�.
		}
		else // DXT ������ �ȵǸ�..
		{
			int nSkip = 0;
			for(int i = 0; i < nMipMapCount; i++)
			{
				...
				if(D3DFMT_DXT1 == fmtOld) nSkip += (sd.Width*2) * (sd.Height*2) * 2 / 4; // DXT1 ������ 1/4 �� ����..
				else nSkip += (sd.Width*2) * (sd.Height*2); // DXT2 ~ DXT5 ������ 1/2 �� ����..
			}
			::SetFilePointer(hFile, nSkip, 0, FILE_CURRENT); // �ǳʶٰ�.
...
bool CN3Texture::Save(HANDLE hFile)
{
		...
		D3DFMT_DXT5 == sd.Format )
	{
		...
		for(int i = 0; i < nMipMapCount; i++)
		{
			...
			m_lpTexture->LockRect(i, &LR, NULL, NULL);
			WriteFile(hFile, (BYTE*)LR.pBits, sd.Size, &dwRWC, NULL); // �Ϸķ� �� �����͸� ����..
			m_lpTexture->UnlockRect(i);
		}
		...
		for(i = 1; i < nMipMapCount; i++)
		{
			...
			D3DXLoadSurfaceFromSurface(lpSurfDest, NULL, NULL, lpSurfSrc, NULL, NULL, D3DX_FILTER_NONE, 0); // ���ǽ� ����.
...
bool CN3Texture::Convert(D3DFORMAT Format, int nWidth, int nHeight)
{
	...
	if(nMMC > 0) this->GenerateMipMap(lpTSOld); // MipMap ����
...
bool CN3Texture::GenerateMipMap(LPDIRECT3DSURFACE9 lpSurf)
{
	...
	int nMMStart = 0;
	if(NULL == lpSurf) // Source Surface �� NULL �̸�..
	{
		nMMStart = 1;
	...
	for(int i = nMMStart; i < nMMC; i++)
	{
		...
		DWORD dwFilter = D3DX_FILTER_NONE; // �⺻ ���ʹ� ����..
		lpSurf->GetDesc(&sdSrc);
		if(sdSrc.Width != sdDest.Width || sdSrc.Height != sdDest.Height) dwFilter = D3DX_FILTER_TRIANGLE; // �ʺ� ���̰� �ٸ���.. ���� ����

		HRESULT rval = D3DXLoadSurfaceFromSurface(lpSurfDest, NULL, NULL, lpSurf, NULL, NULL, dwFilter, 0); // ���� �� ü�ο� ���ǽ� �̹��� ��� ���� 
	...
	if(nMMStart == 1)
	{
		lpSurf->Release(); lpSurf = NULL; // 0 ���� ���ǽ� ������..
	}
