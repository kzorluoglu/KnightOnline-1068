...
{
#ifdef _MAPTOOL	// ���������� �߰��� object�� �����Ͱ� �����������̱� ������ delete �ʰ� �ϱ� ���ؼ�..
	memset(m_pShapes, 0, sizeof(m_pShapes));
	memset(m_pChrs, 0, sizeof(m_pChrs));
#else
	int i;
...
bool CN3SubScene::Load(HANDLE hFile)
{
	...
	int nChrC = 0;
	ReadFile(hFile, &nChrC, sizeof(nChrC), &dwRWC, NULL); // ĳ����
	for(i = 0; i < nChrC; i++)
	{
		ReadFile(hFile, &nL, sizeof(nL), &dwRWC, NULL);
		if(nL < 0) continue;
...
bool CN3SubScene::Save(HANDLE hFile)
{
	...
	WriteFile(hFile, &m_nChrCount, sizeof(m_nChrCount), &dwRWC, NULL); // ĳ����
...
//		{
//			m_fFrmCur += 20.0f / CN3Base::m_fFrmPerSec; // �����ϰ� �����̵��� �ð��� ���� �����̴� ���� ����..
