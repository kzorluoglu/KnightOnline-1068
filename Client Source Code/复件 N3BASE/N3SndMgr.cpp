...
CN3SndObj* CN3SndMgr::CreateObj(const std::string& szFN, e_SndType eType)
{
	...
	if(it == m_SndObjSrcs.end()) // �� ã�Ҵ�... ���� ������..
	{
		...
		if(false == pObjSrc->Create(szFN, eType)) // ���� �ε�..
		{
			delete pObjSrc; pObjSrc = NULL;
			return NULL;
		}
		m_SndObjSrcs.insert(val_Snd(szFN, pObjSrc)); // �ʿ� �߰��Ѵ�..
	}
	...
	if(false == pObjNew->Duplicate(pObjSrc, eType)) // Duplicate ó��..
	{
		delete pObjNew; pObjNew = NULL;
		return NULL;
	}
	
	if(pObjNew) m_SndObjs_Duplicated.push_back(pObjNew); // ����Ʈ�� �ִ´�...
	return pObjNew;
}
...
CN3SndObjStream* CN3SndMgr::CreateStreamObj(const std::string& szFN)
{
	...
	m_SndObjStreams.push_back(pObj); // ����Ʈ�� �ֱ�..
...
{
	if(NULL == ppObj || NULL == *ppObj) return;
	std::string szFN = (*ppObj)->m_szFileName; // ���� �̸��� ����ϰ�..
...
bool CN3SndMgr::PlayOnceAndRelease(int iSndID, const _D3DVECTOR* pPos)
{
	...
	{
		pObjSrc = new CN3SndObj();
		if(false == pObjSrc->Create(pTbl->szFN, (e_SndType)pTbl->iType)) // ���� �ε�..
		{
			delete pObjSrc; pObjSrc = NULL;
			return NULL;
		}
		m_SndObjSrcs.insert(val_Snd(pTbl->szFN, pObjSrc)); // �ʿ� �߰��Ѵ�..
	...
	if(false == pObj->Duplicate(pObjSrc, (e_SndType)pTbl->iType)) // Duplicate ó��..
	{
		delete pObj; pObj = NULL;
		return NULL;
	}
