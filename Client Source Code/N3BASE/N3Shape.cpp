...
CN3SPart::CN3SPart()
{
	...
	m_fTexIndex = 0; // Current Texture Index.. Animation ��ų�� �ʿ��� �ε����̴�..

//	m_vWindFactorCur.Zero();		// ���� �ٶ� �δ� ��.. �̰����� ȸ���� ��Ų��..
//	m_vWindFactorToReach.Zero();	// �ٶ� �δ� ��..
	m_fTimeToSetWind = 0;			// �ٶ� �δ� ���� �ٲٱ� ���� �ð�..
	m_fWindFactorCur = 0;			// ���� �ٶ� �δ� ��.. �̰����� ȸ���� ��Ų��..
	m_fWindFactorToReach = 0;		// �ٶ� �δ� ��..
}
...
void CN3SPart::Tick(const __Matrix44& mtxParent, const __Quaternion& qRot, float fScale) // timeGetTime ���� ���� ���� ������ Texture Animation �� ��Ʈ�� �Ѵ�..
{
	...
	if(s_CameraData.IsOutOfFrustum(vCenter, this->Radius() * fScale)) // ī�޶� ���ü �ٱ��̸� ��������..
	{
...
//	fLOD *= 256.0f / s_CameraData.fFP;
	...
	if(iTC > 1) // �ؽ�ó ���ϸ��̼�
	{
		...
		if(m_fTexIndex >= iTC) m_fTexIndex -= (iTC * m_fTexIndex) / iTC; // ������ ������ �Ҽ����� ����� �ȴ�??(�Ͽ�ư �����~)
	}

	if(m_Mtl.nRenderFlags & RF_BOARD_Y) // ī�޶� �ٶ�����ϴ� �Ÿ�..
	{
	...
	if(m_Mtl.nRenderFlags & RF_WINDY) // �ٶ��� ��¦ ������ �ϸ�..
	{
...
//			m_vWindFactorToReach.y = 0.05f - (0.1f * (rand()%100) / 100.0f); // ���Ʒ��δ� ���ݸ� �Ұ� �Ѵ�.
//			m_vWindFactorToReach.z = 0.05f - (0.1f * (rand()%100) / 100.0f);

			m_fWindFactorToReach = (rand()%100)/100.0f;
			m_fTimeToSetWind = 3.0f * ((rand()%100)/100.0f); // �ٶ��� ���ӵ� ��..
		}
...
bool CN3SPart::Load(HANDLE hFile)
{
	...
	m_TexRefs.clear();
	this->TexAlloc(iTC); // Texture Pointer Pointer �Ҵ�..
	for(int j = 0; j < iTC; j++) // Texture Count ��ŭ ���� �̸� �о �ؽ�ó �θ���..
	{
...
bool CN3SPart::Save(HANDLE hFile)
{
	...
	else MessageBox(GetActiveWindow(), "Progressive mesh pointer is NULL! : object�� ����� ������ ���� �� �ֽ��ϴ�.(���ҽ� ������ Load���� �ʾ��� ���ɼ��� ŭ)", "warning", MB_OK);

	WriteFile(hFile, &nL, 4, &dwRWC, NULL); // Mesh FileName
	if(nL > 0)
	{
		
//		if(-1 == pPMesh->FileName().find("object\\")) // �ӽ÷� ��θ� �ٲٷ��� �־���.. ���߿� �ʿ���� �����..
//		{
//			char szFNTmp[256];
...
//		}
		...
		WriteFile(hFile, pPMesh->FileName().c_str(), nL, &dwRWC, NULL); // �޽� ���� �̸�..
	}
	...
	for(int j = 0; j < iTC; j++) // Texture File �̸� ����...
	{
...
//			if(-1 == m_TexRefs[j]->FileName().find("object\\")) // �ӽ÷� ��θ� �ٲٷ��� �־���.. ���߿� �ʿ���� �����..
//			{
//				// ���� �̸��� �и��ϰ�..
...
//			}
			...
			WriteFile(hFile, m_TexRefs[j]->FileName().c_str(), nL, &dwRWC, NULL); // �ý�ó ���� �̸�..
		}
	}
...
CN3Shape::CN3Shape()
{
	...
	m_iNPC_ID = 0;  // NPC �� ���� ������Ʈ�� ��� NPC ID
	m_iNPC_Status = 0; // NPC �� ���� ������Ʈ�� ��� NPC Status
}
...
bool CN3Shape::Load(HANDLE hFile)
{
	...
	if(iPC > 0)
	{
		...
		for(int i = 0; i < iPC; i++)
		{
			...
			m_Parts[i]->ReCalcMatrix(m_Matrix); // Part Matrix ���
		}
	}
		
	ReadFile(hFile, &m_iBelong, 4, &dwRWC, NULL);	// �Ҽ�
	ReadFile(hFile, &m_iEventID, 4, &dwRWC, NULL);	// Event ID
	ReadFile(hFile, &m_iEventType, 4, &dwRWC, NULL); // Event Type - ���ε� ����Ʈ, ����, ���� ���...
	ReadFile(hFile, &m_iNPC_ID, 4, &dwRWC, NULL);	// NPC �� ���� ������Ʈ�� ��� NPC ID
	ReadFile(hFile, &m_iNPC_Status, 4, &dwRWC, NULL); // NPC �� ���� ������Ʈ�� ��� NPC Status
...
{
	return CN3Transform::Load(hFile); // �⺻���� �б�...
}
...
bool CN3Shape::Save(HANDLE hFile)
{
	...
	WriteFile(hFile, &m_iBelong, 4, &dwRWC, NULL);		// �Ҽ�
	WriteFile(hFile, &m_iEventID, 4, &dwRWC, NULL);		// Event ID
	WriteFile(hFile, &m_iEventType, 4, &dwRWC, NULL);		// Event Type - ���ε� ����Ʈ, ����, ���� ���...
	WriteFile(hFile, &m_iNPC_ID, 4, &dwRWC, NULL);		// NPC �� ���� ������Ʈ�� ��� NPC ID
	WriteFile(hFile, &m_iNPC_Status, 4, &dwRWC, NULL);	// NPC �� ���� ������Ʈ�� ��� NPC Status
...
void CN3Shape::FindMinMax()
{
	...
	for(int i = 0; i < iPC; i++)
	{
		vMinTmp = m_Parts[i]->Min() * mtxWI; // ���� ���� �ּҰ��� ���� ��ǥ�� �ٲپ��ش�..
		vMaxTmp = m_Parts[i]->Max() * mtxWI; // ���� ���� �ִ밪�� ���� ��ǥ�� �ٲپ��ش�..
...
{
	if(false == bIgnoreBoxCheck && false == ::_CheckCollisionByBox(vPos, vDir, m_vMin * m_Matrix, m_vMax * m_Matrix)) return -1; // �ڽ� üũ �����Ѵ�..
	...
	for(int i = 0; i < iPC; i++)
	{
		...
		if(nFC > 64 && false == ::_CheckCollisionByBox(vPos, vDir, m_Parts[i]->Min(), m_Parts[i]->Max())) continue;  // Face ���� 24 ������ ���� ��� �ϴ� �ڽ�üũ�� �Ѵ�..
		...
		vPos2 = vPos * mtxWI;
		mtxWI.PosSet(0,0,0);
		vDir2 = vDir * mtxWI; // ����ķ� ȸ��..

		int nCI0, nCI1, nCI2;
		__Vector3 v0, v1, v2;
		for(int j = 0; j < nFC; j++) // ������ Face ���� �浹üũ..
		{
			nCI0 = pwIs[j*3+0];
			nCI1 = pwIs[j*3+1];
			nCI2 = pwIs[j*3+2];
...
bool CN3Shape::MakeCollisionMeshByParts()  // �浹 �޽ø� �ڽ��� �����...
{
	int iPC = m_Parts.size();
	int iVC = 0, iIC = 0;
	for(int i = 0; i < iPC; i++)
	...
	char szBuff[256];
	sprintf(szBuff, "%s_collision_%d.n3vmesh", m_szFileName.c_str(), iCount); // �ӽ÷� �̸��� ����..
...
bool CN3Shape::MakeCollisionMeshByPartsDetail()  // ���� ��� �״��... �浹 �޽ø� �����...
{
	int iPC = m_Parts.size();
...
bool CN3Shape::SaveToSameFolder(const std::string& szFullPath)
{
	...
	for(int i = 0; i < iPC; i++)
	{
		...
		OldPartFNs.push_back(szOldFN); // ���� �̸� ����..
...
//	By : Ecli666 ( On 2002-10-16 ���� 11:44:19 )
//
...
//	By : Ecli666 ( On 2002-08-06 ���� 4:33:32 )
//
