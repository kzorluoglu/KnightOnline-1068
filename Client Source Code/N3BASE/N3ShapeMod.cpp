...
BOOL CN3ShapeMod::LoadStateInfo(FILE* stream)	// ���� ������ �о�´�.(text�κ���)
{
	__ASSERT(m_Parts.size() > 0, "���� shape�� load�ؾ� �Ѵ�.");
	if (NULL == stream) return FALSE;
//	Release();

	int result;
//	char szSrcName[_MAX_PATH];	// shape source���� �̸�
//	result = fscanf(stream, "Shape_FName=%s\n", szSrcName);	__ASSERT(result != EOF, "�߸��� N3ShapeMod ���� ����");

	// shape load�ϱ�
//	if (!this->Load(szSrcName)) return FALSE;

	// ��ȭ�ϴ� PMesh�� ���� �� State�� ��
	result = fscanf(stream, "PMesh_Count=%d\n", &m_iModPartCount);	__ASSERT(result != EOF, "�߸��� N3ShapeMod ���� ����");
	result = fscanf(stream, "State_Count=%d\n", &m_iStateCount);		__ASSERT(result != EOF, "�߸��� N3ShapeMod ���� ����");
...
//	{
//		result = fscanf(stream, "S_Pos(%f, %f, %f)\n", &vPos.x, &vPos.y, &vPos.z);		__ASSERT(result != EOF, "�߸��� N3ShapeMod ���� ����");
//		result = fscanf(stream, "S_Rot(%f, %f, %f)\n", &vRot.x, &vRot.y, &vRot.z);		__ASSERT(result != EOF, "�߸��� N3ShapeMod ���� ����");
//		result = fscanf(stream, "S_Scale(%f, %f, %f)\n", &vScale.x, &vScale.y, &vScale.z);		__ASSERT(result != EOF, "�߸��� N3ShapeMod ���� ����");
...
//	}
	...
	// Part ���� ���� �о����
	__ASSERT(m_iModPartCount > 0, "m_iModPartCount�� 1�̻��̾�� �մϴ�.");
	__ASSERT(m_iStateCount > 0, "m_iStateCount 1�̻��̾�� �մϴ�.");
	m_pModParts = new __ModPart[m_iModPartCount];

	char szPMeshName[_MAX_PATH] = "";
	for (i=0; i<m_iModPartCount; ++i)
	{
		result = fscanf(stream, "PMesh_FName=%s\n", szPMeshName);		__ASSERT(result != EOF, "�߸��� N3ShapeMod ���� ����");
		...
		for (j=0; j<m_iStateCount; ++j)
		{
			...
			result = fscanf(stream, "Rot(%f, %f, %f, %f)\n", &vAxis.x, &vAxis.y, &vAxis.z, &fDegree);		__ASSERT(result != EOF, "�߸��� N3ShapeMod ���� ����");
	...
	// m_pMatchPart2ModPart CN3SPart������ŭ ���� �� �ʱ�ȭ(��Ī��Ű��)
	__ASSERT(m_pMatchPart2ModPart == NULL, "Machine���� �޸� �� ���ɼ�");
