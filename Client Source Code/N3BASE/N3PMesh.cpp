...
//		m_pIndices = new WORD[m_iMaxNumIndices];
	...
	{
		m_pCollapses = new __EdgeCollapse[m_iNumCollapses+1];	// +1�� �� ���� : PMeshInstance::SplitOne() �Լ����� �ε����ϰ� �����Ͱ� ��輱�� ����Ű�� �ؾ� �ϴ� ��찡 �־.
		ReadFile(hFile, m_pCollapses, m_iNumCollapses*sizeof(__EdgeCollapse), &dwNum, NULL);
		ZeroMemory(m_pCollapses + m_iNumCollapses, sizeof(__EdgeCollapse));	// ���� +1�� �������� ����. ������ ����� ������ ����Ÿ�� �ʱ�ȭ �ص�
...
#ifdef _DEBUG
		if(bFixed)
			::MessageBox(s_hWndBase, "�߸��� Progressive Mesh ����", m_szName.c_str(), MB_OK);
#endif
	}
...
bool CN3PMesh::Save(HANDLE hFile)
{
	...
	{
		for(int i = 0; i < m_iNumCollapses; i++)
			if(m_pCollapses[i].iIndexChanges < 0) m_pCollapses[i].iIndexChanges = 0; // ����..
...
void CN3PMesh::ReGenerateSmoothNormal()
{
	...
	PMI.SetLODByNumVertices(m_iMaxNumVertices); // �ִ� ������ �����ϰ�..
