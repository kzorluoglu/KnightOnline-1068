...
void CN3AnimKey::Alloc(int nCount, float fSamplingRate, ANIMATION_KEY_TYPE eType)
{
	...
	{
		__ASSERT(0, "Animation Sampling Rate �� �ݵ�� 0 ���� Ŀ�� �մϴ�.");
		return;
	}
	...
	{
		m_pDatas = new __Vector3[nCount+1]; // �Ѱ� �� �����ְ� �Ҵ�.
		memset(m_pDatas, 0, sizeof(__Vector3) * (nCount+1));
	}
	else if(KEY_QUATERNION == m_eType)
	{
		m_pDatas = new __Quaternion[nCount+1]; // �Ѱ� �� �����ְ� �Ҵ�.
...
bool CN3AnimKey::Load(HANDLE hFile)
{
	...
	DWORD dwRWC = 0;
	ReadFile(hFile, &m_nCount, 4, &dwRWC, NULL); // Ű�� � �ִ���
...
{
	DWORD dwRWC = 0;
	WriteFile(hFile, &m_nCount, 4, &dwRWC, NULL); // Ű�� � �ִ���
...
void CN3AnimKey::Add(CN3AnimKey &AKSrc, int nIndexS, int nIndexE)
{
	...
	int nAddCount = nIndexE - nIndexS + 1; // �߰��� ����
	...
	this->Alloc(nPrevCount + nAddCount, AKSrc.SamplingRate(), AKSrc.Type()); // ���� �Ҵ�.
	if(nPrevCount > 0)
	{
		if(KEY_VECTOR3 == m_eType)
		{
