...
bool CN3SPartEx::Load(HANDLE hFile)
{
	...
	int iStateCount = 0;
	if (m_ActionInfo.dwActionFlag) ReadFile(hFile, &iStateCount, sizeof(iStateCount), &dwRWC, NULL);	// ���� ������ ����
	if (AF_POS & m_ActionInfo.dwActionFlag)
	{
		m_ActionInfo.ActionStateInfos_Pos.assign(iStateCount);
		for (int i=0; i<iStateCount; ++i)
			ReadFile(hFile, &(m_ActionInfo.ActionStateInfos_Pos[i]), sizeof(__Vector3), &dwRWC, NULL);	// ���� ������ ����
	}
	if (AF_SCALE & m_ActionInfo.dwActionFlag)
	{
		m_ActionInfo.ActionStateInfos_Scale.assign(iStateCount);
		for (int i=0; i<iStateCount; ++i)
			ReadFile(hFile, &(m_ActionInfo.ActionStateInfos_Scale[i]), sizeof(__Vector3), &dwRWC, NULL);	// ���� ������ ����
	}
	if (AF_ROTATION & m_ActionInfo.dwActionFlag)
	{
		m_ActionInfo.ActionStateInfos_Rot.assign(iStateCount);
		for (int i=0; i<iStateCount; ++i)
			ReadFile(hFile, &(m_ActionInfo.ActionStateInfos_Rot[i]), sizeof(__Quaternion), &dwRWC, NULL);	// ���� ������ ����
	}
...
bool CN3ShapeEx::Load(HANDLE hFile)
{
	...
	DWORD dwRWC;
	ReadFile(hFile, &m_iActionStateCount, sizeof(m_iActionStateCount), &dwRWC, NULL);	// ���� ������ ����
...
/*
	// m_Matrix �ٽ� ���..
	CN3Transform::ReCalcMatrix(); // Transform Matrix �� ��� ���ְ�..
