...
void CN3PMeshCreate::combine_modified(float &sofar, WORD *tri, int which, int what_to, bool bSumOfLoss)
{
	...
	if (weighted_angle_change<0.0f) weighted_angle_change = 0.0f;	// cosangdiff�� 1���� ���� ���� Ŭ���� �־ weighted_angle_change�� -���� ���ö��� �ִ�.
	//__ASSERT(weighted_angle_change>=0.0f, "weighted_angle_change > 0 �̾�� �Ѵ�.");
					 ...
					 bool &IsOver)
{
	...
	for (WORD *tri = m_pIndices; tri < m_pIndices + m_iNumIndices; tri += 3)
	{
		if (tri[0] == pt_a)
		{
			if (tri[1] != pt_b && tri[2] != pt_b)
			{
				...
				__ASSERT(a_loss>=0.0f, "loss > 0�̾�� �Ѵ�.");
			}
			else
			{
				// tri collapses.
				if (m_PMCOption.bTriangleWeight)
				{
					...
					__ASSERT(a_loss>=0.0f && b_loss>=0.0f, "loss > 0�̾�� �Ѵ�.");
				}
				continue;
			}
		}
		...
		else
		{
			...
			if (tri[0] == pt_b)
			{
				...
				__ASSERT(b_loss>=0.0f, "loss > 0�̾�� �Ѵ�.");
			}
			else
			if (tri[1] == pt_b)
			{
	...
	__ASSERT(temp_a_loss>=0.0f && temp_b_loss>=0.0f, "loss > 0�̾�� �Ѵ�.");
	if (m_PMCOption.bUseSumOfLoss)
	{
		a_loss += temp_a_loss;
		b_loss += temp_b_loss;
...
int CN3PMeshCreate::ReGenerate(CN3PMesh *pPMesh)
{
	...
	this->Release();
	this->ConvertFromN3PMesh(pPMesh); // Mesh �� ���� �����..
	this->CreateCollapseList(); // Progressive Mesh ó��..
...
// swapper��° ���ؽ��� ���ؽ������� m_iNumVertices-1 ��°�� ������
// ( m_iNumVertices�� collapse ����Ʈ�� ���鶧���� �ϳ��� ����) m_iNumVertices-1��°�� swapper��°�� �ű��.
// �ε��� ����, collapse����Ʈ ���� ���� �ε���, pt_to, pt_end�� ���ο� �ε����� �°� swap �ϴ� �Լ�
...
bool CN3PMeshCreate::ConvertFromN3Mesh(CN3Mesh* pN3Mesh)	// N3Mesh -> CN3PMeshCreate �� ������..
{
...
bool CN3PMeshCreate::ConvertFromN3PMesh(CN3PMesh* pN3PMesh)
{
	...
	CN3PMesh* pPMeshTmp = CN3Base::s_MngPMesh.Get(pN3PMesh->FileName()); // �̷��� ���� ī��Ʈ�� �ϳ� �þ �����Ͱ� �Ⱦ�������.
