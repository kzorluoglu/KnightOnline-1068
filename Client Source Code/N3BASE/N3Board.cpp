...
void CN3Board::LoadFromText(const std::string& szFName)
{
	...
	result = fscanf(stream, "Position = %f %f %f\n", &(vPos.x), &(vPos.y), &(vPos.z));	__ASSERT(result != EOF, "�߸��� Machine ���� ����");
	result = fscanf(stream, "Size = %f %f\n", &fWidth, &fHeight);	__ASSERT(result != EOF, "�߸��� Machine ���� ����");
	result = fscanf(stream, "Rotation Axis = %s\n", szBoardType);		__ASSERT(result != EOF, "�߸��� Machine ���� ����");
	result = fscanf(stream, "Frame per Sec = %f\n", &m_fTexFPS);			__ASSERT(result != EOF, "�߸��� Machine ���� ����");

	result = fscanf(stream, "Render Flag = %d\n", &m_Mtl.nRenderFlags);		__ASSERT(result != EOF, "�߸��� Machine ���� ����");
	result = fscanf(stream, "Source Blend = %d\n", &m_Mtl.dwSrcBlend);		__ASSERT(result != EOF, "�߸��� Machine ���� ����");
	result = fscanf(stream, "Dest Blend = %d\n", &m_Mtl.dwDestBlend);		__ASSERT(result != EOF, "�߸��� Machine ���� ����");
	// �Ȱ�, culling �ɼ��� �켱 ������ ����.

	result = fscanf(stream, "Texture Count = %d\n", &iCount);		__ASSERT(result != EOF, "�߸��� Machine ���� ����");
	...
	if (iCount>0)
	{
		...
		for (i=0; i<iCount; ++i)
		{
			result = fscanf(stream, "Texture Name = %s\n", &szTexFName);		__ASSERT(result != EOF, "�߸��� Machine ���� ����");
			TexSet(i, szTexFName);
		}
	}
