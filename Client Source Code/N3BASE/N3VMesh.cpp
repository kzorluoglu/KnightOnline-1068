...
bool CN3VMesh::CheckCollision(const __Matrix44& MtxWorld, const __Vector3& v0, const __Vector3& v1, __Vector3* pVCol, __Vector3* pVNormal)
{
	...
	for(int i = 0; i < nFC; i++)
	{
		...
		if(false == ::_IntersectTriangle(vPos0, vDir, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2], fT, fU, fV, &vColTmp)) continue; // ù° ���Ͱ� ��ġ��..
		if(false == ::_IntersectTriangle(vPos1, vDir, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2])) // ��°�� �� ��ġ��..
		{
			fDistTmp = (vPos0 - vColTmp).Magnitude(); // �Ÿ��� �纸��..
