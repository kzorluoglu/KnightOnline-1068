...
{
	m_qOrient.Identity();		// Joint Orient Quaternion
	m_KeyOrient.Release();		// Joint Orient Ű��... NULL �̸� ���°Ŵ�..
...
#ifdef _N3TOOL
BOOL CN3Joint::FindPointerByName(const std::string& szName, CN3Joint *&pJoint) // �̸��� ������ �ش� ����� �����͸� �����ش�..
{
	if(szName.empty())
	{
		pJoint = NULL;
...
//	}
	...
	if(m_KeyOrient.Count() > 0) // Orient Ű���� ������..
	{
		qt = m_qRot * m_qOrient;
		::D3DXMatrixRotationQuaternion(&m_Matrix, &qt); // ȸ��.. ���ʹϾ� ���..
	}
	else
	{
...
void CN3Joint::AddKey(CN3Joint *pJSrc, int nIndexS, int nIndexE)
{
	...
	for(int i = 0; i < iSize; i++, it++, it2++)
	{
		...
		pChildDest->AddKey(pChildSrc, nIndexS, nIndexE); // ���ȣ��
	}
}
#endif // end of _N3TOOL
...
void CN3Joint::CopyExceptAnimationKey(CN3Joint *pJSrc)
{
	...
	for(; it != itEnd; it++)
	{
		...
		this->ChildAdd(pChildDest); // �ڽ� �߰�..

		CN3Joint* pChildSrc = *it;
		pChildDest->CopyExceptAnimationKey(pChildSrc); // ���ȣ��
	}
}
#endif // end of _N3TOOL
