...
//                                                      D3DPOOL_MANAGED, &m_pVB ) ) )
    if( FAILED( hr = m_pd3dDevice->CreateVertexBuffer( iVBSize, 0, FVF_TRANSFORMED, D3DPOOL_MANAGED, &m_pVB, NULL ) ) )
    {
        return hr;
    }
...
//	szTemp[iTempCount] = 0x00;
	...
	SIZE size2;	// �ѱ� �ݱ����� ũ��..
	GetTextExtentPoint32( s_hDC, "��", lstrlen("��"), &size2 );
	...
	{
		int iMipMapCount = 1;
		if( dwFlags & D3DFONT_FILTERED ) iMipMapCount = 0; // ���͸� �ؽ�Ʈ�� �Ӹ��� �����..
...
// �۾� ��� �� �۾� ���� �Ƕ��� �����
	...
	::SelectObject(s_hDC, hObjPrev); // �ݵ�� ���ǰ� �����ؾ�..
...
void CDFont::Make2DVertex(const int iFontHeight, const std::string& szText)
{
	...
	char	szTempChar[3] = "";
	DWORD dwColor = 0xffffffff;			// ��Ʈ�� ��
	m_dwFontColor = 0xffffffff;
	SIZE size;

	float fMaxX = 0.0f, fMaxY = 0.0f;	// �۾��� ������ ������ �ִ� �ּҰ��� �����ϱ� ���ؼ�.
	...
	while(iCount<iStrLen)
	{
		if ('\n' == szText[iCount])		// \n
		{
			...
			if (sx != x)
			{
				...
				__ASSERT(dwNumTriangles+2 < MAX_NUM_VERTICES, "??");		// Vertex buffer�� ���ڶ���.
...
void CDFont::Make3DVertex(const int iFontHeight, const std::string& szText, DWORD dwFlags)
{
	...
	int i;
	if (dwFlags & D3DFONT_CENTERED)	// ��� �����̸� vertex��ǥ�� ����� ����ؼ� ���ĳֱ�
	{
		// ���� �� �� ã��..
	...
	for (i=0; i<iCount; ++i)
	{
		TempVertices[i].x /= ((float)m_dwFontHeight);			// ���� ũ��� ���̱�
		TempVertices[i].y /= ((float)m_dwFontHeight);			// ���� ũ��� ���̱�
...
//	}
//	else if ( D3DZB_TRUE != dwZEnable) m_pd3dDevice->SetRenderState( D3DRS_ZENABLE, D3DZB_TRUE );	// fZ�� 1.0�� �ƴϸ� z ���� �Ѱ� �׸���.
...
//	else if ( D3DZB_TRUE != dwZEnable) m_pd3dDevice->SetRenderState( D3DRS_ZENABLE, dwZEnable );
	...
	{
		if (D3DTEXF_POINT != dwMinFilter) m_pd3dDevice->SetSamplerState( 0, D3DSAMP_MINFILTER, dwMinFilter );
		if (D3DTEXF_POINT != dwMagFilter) m_pd3dDevice->SetSamplerState( 0, D3DSAMP_MAGFILTER, dwMagFilter );
	}
...
//			}
	...
	if(!(dwFlags & D3DFONT_FILTERED)) pAP->nRenderFlags |= RF_POINTSAMPLING; // ���͸� �ؽ�Ʈ�� ���� �ʴ´�.
}
