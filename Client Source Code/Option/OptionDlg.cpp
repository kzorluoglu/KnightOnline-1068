...
BOOL COptionDlg::OnInitDialog()
{
	...
	lStatus = RegQueryValueEx(hRegKey, "PATH", NULL, &dwType, (BYTE*)szBuff, &dwBytes); // �ν��� ���
	...
	// ���� ���� �̸�
//	dwType = REG_SZ; dwBytes = 256;
//	lStatus = RegQueryValueEx(hRegKey, "EXE", NULL, &dwType, (BYTE*)szBuff, &dwBytes); // �������� �̸�
...
//	m_szExeName = szBuff;
	...
	lStatus = RegQueryValueEx(hRegKey, "VERSION", NULL, &dwType, (BYTE*)(&dwVersion), &dwBytes); // ����
...
{
	CString szExeFN = m_szInstalledPath + "\\" + m_szExeName; // ���� ���� �̸� �����..
	ShellExecute(NULL, "open", szExeFN, "", m_szInstalledPath, SW_SHOWNORMAL); // ���� ����..
...
void COptionDlg::OnBVersion() 
{
	...
	if(IDNO == MessageBox(szMsg, "", MB_YESNO)) return; // �ѹ� �����..
	...
	if(ERROR_SUCCESS == lStatus)
	{
		...
		lStatus = RegSetValueEx(hRegKey, "VERSION", NULL, dwType, (BYTE*)(&dwVersion), 4); // ����
