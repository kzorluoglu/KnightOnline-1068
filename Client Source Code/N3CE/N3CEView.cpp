...
{ 
	...
	ASSERT_VALID(pDoc);
	if(TRUE == pDoc->m_bLoadingNow) return; // �д� ���߿��� ���������� �ʴ´�...


	float fFrm = pDoc->m_Scene.m_fFrmCur; // �Ϻη� �������� ��Ʈ�� �Ϸ��� �̷��� �س��Ҵ�..
	pDoc->m_Scene.TickCameras(fFrm); // ī�޶� Tick
	...
	if(m_bRenderOptionXRay) // ������ �ɼ��� ���� ������..
	{
		...
		pFrm->m_Eng.s_lpD3DDev->SetRenderState(D3DRS_TEXTUREFACTOR, 0x80000000);	// alpha factor ����
...
//		m_pJointSelected->CN3Transform::Render(&(pChr->m_Matrix), 0.3f); // �� �׸���..
	...
	if(m_bPlayingNow)
	{
		...
		{
			pChr->PosSet(0,0,0);
			if(!m_DequeAnimation.empty()) // ���ϸ��̼� ť�� ������..
			{
	...
	if(hItem)
	{
		...
		else if(pBase && (pBase->Type() & OBJ_CHARACTER_PLUG) )
		{
			...
			if(pChr->MatrixGet(pPlug->m_nJointIndex))
			{
				...
				pPlug->RenderFXLines(pChr->m_Matrix, mtxJ); // FX �� ���� ���� �׷��ش�.
			}
		}
	}
	// Wire Frame Render
	...
	if(eCM_PlugFXPosition0 == m_eCursorMode ||
		...
		eCM_PlugFXPosition7 == m_eCursorMode ) // �÷��� ȿ�� ��ġ 0
	{
		m_pPosDummy->Tick();
		m_pPosDummy->Render();
	}
...
void CN3CEView::OnLButtonDown(UINT nFlags, CPoint point) 
{
		...
		!::_IsKeyDown(VK_MENU))
	{
		...
		if(pJoint)
		{
			...
			CN3Base* pBase = pFrm->GetPaneProperty()->GetSelectedObject(); // Plug �� Joint Index �� �ִ´�..
			if(	eCM_PickJoint == this->m_eCursorMode && // ����Ʈ ���� ���..
...
//////////////////////////////////////////////////
//	Coded (By Dino On 2002-10-11 ���� 1:50:47 )
//	FXPlug
			else if (pBase && (pBase->Type() & OBJ_FX_PLUG_PART)	// FXPlugPart�� ���õǾ� ������� �����ϴ� ����Ʈ�� �����Ѵ�.
				&&( nJI >= 0 && nJI < n))
			{
...
//	End Of Code (By Dino On 2002-10-11 ���� 1:50:47 )
//////////////////////////////////////////////////
...
void CN3CEView::OnMouseMove(UINT nFlags, CPoint point) 
{
	if(	!::_IsKeyDown(VK_MENU) && 
		((nFlags & MK_LBUTTON) || (nFlags & MK_MBUTTON) || (nFlags & MK_RBUTTON)) ) // Ŀ�� ��忡 ���� ����..
	{
		// �ϴ� �÷��װ� ���õǾ����� ����.
		CMainFrame* pFrm = (CMainFrame*)AfxGetMainWnd();
		CN3Base* pBase = pFrm->GetPaneProperty()->GetSelectedObject();
		CN3Chr* pChr = GetDocument()->m_Scene.ChrGet(0);
		if(	eCM_Nothing != m_eCursorMode && // Ŀ�� ��尡 ����� �ƴϰ�..
			pChr && pBase && (pBase->Type() & OBJ_CHARACTER_PLUG))
		{
			...
			if(eCM_PlugPosition == m_eCursorMode) // �÷��� ��ġ
			{
			...
			else if(eCM_PlugScale == m_eCursorMode) // �÷��� ������
			{
				CPoint ptDelta = point - m_ptPrev;
			...
			else if(eCM_PlugRotation == m_eCursorMode) // �÷��� ������
			{
				const __Matrix44* pMtxTmp = pChr->MatrixGet(pPlug->m_nJointIndex);
				if(pMtxTmp)
				{
...
//////////////////////////////////////////////////
//	Coded (By Dino On 2002-10-11 ���� 2:04:29 )
//	FXPlug
...
//	End Of Code (By Dino On 2002-10-11 ���� 2:04:29 )
//////////////////////////////////////////////////
	}
