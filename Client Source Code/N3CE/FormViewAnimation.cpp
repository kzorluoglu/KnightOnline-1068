...
const float FRAME_PRECISION = 10.0f; // ������ ǥ�� ���е�
...
//			if(1 == pAniData->iBlendFlags) fTime += pAniData->fTimeBlend;
			CString szTime; szTime.Format("�ð� : %.2f��", fTime);
			SetDlgItemText(IDC_STATIC_ANIMATION_TIME_SUM, szTime);
		}
		else
		{
			SetDlgItemText(IDC_STATIC_ANIMATION_TIME_SUM, "�ð� : ??��");
		}
	}
	else
	{
...
void CFormViewAnimation::UpdateInfo()
{
	...
	if(pAniData)
	{
		...
		CheckDlgButton(IDC_C_DELAYED_LOOPING, pAniData->iBlendFlags);  // �����Ҷ� ������ �ð���ŭ ������Ű�鼭 �����Ѵ�..
...
void CFormViewAnimation::GetData()
{
	...
	if(IsDlgButtonChecked(IDC_C_DELAYED_LOOPING)) pAniData->iBlendFlags = 1; // �����Ҷ� ������ �ð���ŭ ������Ű�鼭 �����Ѵ�..
...
void CFormViewAnimation::OnSelchangeListAnimation0() 
{
	...
	{
		pChr->AniCurSet(iAni); // ���ϸ��̼� ����..
	}

	::SetFocus(GetDlgItem(IDC_E_ANI_NAME)->m_hWnd);

	GetDocument()->m_Scene.m_fFrmCur = pAniData->fFrmStart; // ������ ���߰�..
...
void CFormViewAnimation::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) 
{
	...
	if((void*)pScrollBar == (void*)&m_SldFrm)
	{
		...
		m_SldSceneFrm.SetPos(fFrm * FRAME_PRECISION); // Scene �����̴��� �������ش�.
	}
...
void CFormViewAnimation::OnSize(UINT nType, int cx, int cy) 
{
	...
	if(m_SldSceneFrm.GetSafeHwnd() != NULL)
	{
		...
		m_SldSceneFrm.GetWindowRect(rcOrg); // �׷��� ��Ʈ���� ���� ȭ�� ��ǥ�� 
...
//	{
//		CRect rcOrg, rcClient;
//		m_SldFrm.GetWindowRect(rcOrg); // �׷��� ��Ʈ���� ���� ȭ�� ��ǥ�� 
//		this->GetClientRect(rcClient); // 
//		this->ClientToScreen(rcClient); // ȭ�� ��ǥ�� �ٲٰ�..
//
//		int cx2 = rcClient.right - rcOrg.left - 5;
...
//	}
	...
	{
		CRect rcOrg, rcClient;
		m_ListAnim0.GetWindowRect(rcOrg); // �׷��� ��Ʈ���� ���� ȭ�� ��ǥ�� 
		this->GetClientRect(rcClient); // 
		this->ClientToScreen(rcClient); // ȭ�� ��ǥ�� �ٲٰ�..
	...
	{
		CRect rcOrg, rcClient;
		m_ListAnim1.GetWindowRect(rcOrg); // �׷��� ��Ʈ���� ���� ȭ�� ��ǥ�� 
		this->GetClientRect(rcClient); // 
		this->ClientToScreen(rcClient); // ȭ�� ��ǥ�� �ٲٰ�..
...
void CFormViewAnimation::OnEditAnimationDataMoveUp() 
{
	...
	pAniCtrl->Swap(iAni, iAniToChange); // ���Ŷ� �Ʒ����� �ٲ۴�.
...
void CFormViewAnimation::CalculateDelayTimeAndUpdate(int iAnimationIndexOffset)
{
	...
	{
		MessageBox("Animation Data �� ��� �ӵ��� 0 �Դϴ�.");
	}
}
