...
void CPropertyView::OnInitialUpdate() 
{
	...
	if(FALSE == bInitCtrl)
	{
		...
		m_UIImage.AddPropItem("Animate Frame", "", PIT_EDIT, "");			// Animate frame �ʴ� �� ���������� ���ϸ���Ʈ �� ���ΰ�?
		...
		// progress
		m_UIProgress.AddPropItem("Style","", PIT_COMBO, "������ ����|���� ����|�Ʒ��� ����|���� ����|");
		...
		// trackbar
		m_UITrackBar.AddPropItem("Style","", PIT_COMBO, "����|����|");
		...
		// scrollbar
		m_UIScrollBar.AddPropItem("Style","", PIT_COMBO, "����|����|");
	...
	if((void*)wParam == (&m_UIImage) && UI_TYPE_IMAGE == pUI->UIType())
	{
		...
		{	// Animation image�� �����
			...
			while(1)
			{
				...
				{	// �ݵ�� �θ�� image�� �ƴϾ�� �Ѵ�.
					pFrm->MessageBox("Animate image�� �ڽ� image�� animate image�� ���� �� �����ϴ�.");
					break;	
				}
				CDlgAnimate dlgAnim;
				if (IDCANCEL == dlgAnim.DoModal())	break;// animate�Ǵ� �׸��� �����ΰ�?
				pImage->SetAnimImage(dlgAnim.m_iCount);	// ��� ����
				if (dlgAnim.m_iCount<=0)	break; // 1�� �̻��̸� texture�� uv��ǥ ����
				char szTexFName[_MAX_PATH];
				if (FALSE == SelectTexture(szTexFName))	break;// texture�̸� ���ϱ�
				// �������� �̹��� �����ϰ� �ϱ�
				CDlgTexture	dlgTex;
				...
				if (IDCANCEL == dlgTex.DoModal()) break;	// ����� ���
...
{
	CPoint ptWndPos = -GetScrollPosition();
	if (m_UIBase.IsWindowVisible()) SetPropertyListPos(&m_UIBase, ptWndPos);	// Property list window ��ġ����
	if (m_UIImage.IsWindowVisible()) SetPropertyListPos(&m_UIImage, ptWndPos);	// Property list window ��ġ����
	if (m_UIString.IsWindowVisible()) SetPropertyListPos(&m_UIString, ptWndPos);	// Property list window ��ġ����
	if (m_UIButton.IsWindowVisible()) SetPropertyListPos(&m_UIButton, ptWndPos);	// Property list window ��ġ����
	if (m_UIStatic.IsWindowVisible()) SetPropertyListPos(&m_UIStatic, ptWndPos);	// Property list window ��ġ����
	if (m_UIEdit.IsWindowVisible()) SetPropertyListPos(&m_UIEdit, ptWndPos);	// Property list window ��ġ����
	if (m_UIProgress.IsWindowVisible()) SetPropertyListPos(&m_UIProgress, ptWndPos);	// Property list window ��ġ����
	if (m_UITrackBar.IsWindowVisible()) SetPropertyListPos(&m_UITrackBar, ptWndPos);	// Property list window ��ġ����
	if (m_UIScrollBar.IsWindowVisible()) SetPropertyListPos(&m_UIScrollBar, ptWndPos);	// Property list window ��ġ����
	if (m_UIArea.IsWindowVisible()) SetPropertyListPos(&m_UIArea, ptWndPos);	// Property list window ��ġ����
...
void CPropertyView::UpdateUIStringInfo()
{
	...
	if (pItem)
	{
		...
		if ( pUI->m_pDFont)
		{
			...
			if (D3DFONT_BOLD & dwFlag)	pItem->m_curValue += "���� ";
			if (D3DFONT_ITALIC & dwFlag)pItem->m_curValue += "����Ӳ�";
		}
		if (pItem->m_curValue.GetLength()==0) pItem->m_curValue = _T("����");
	}
...
void CPropertyView::UpdateUIListInfo()
{
	...
	{
		pItem->m_curValue = _T("");
		if (pUI->FontIsBold())	pItem->m_curValue += "���� ";
		if (pUI->FontIsItalic()) pItem->m_curValue += "����Ӳ�";
		if (pItem->m_curValue.GetLength()==0) pItem->m_curValue = _T("����");
	}
...
void CPropertyView::UpdateUIStaticInfo()
{
	...
	if (UI_TYPE_STATIC == pUIBase->UIType() ||
		UI_TYPE_EDIT == pUIBase->UIType() ) pUI = (CN3UIStatic*)pUIBase;	// edit�� static���� ��� ���� ���̴�.
	else return;
