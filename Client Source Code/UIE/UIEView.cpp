...
void CUIEView::OnDraw(CDC* pDC)
{
	...
	if (UIEMODE_PREVIEW == m_eMode) return;	// preview�϶��� �׳� ����
	...
	if(RT_NONE != m_eSelectedRectType)// Rect �������϶� ���� Rect ǥ��
	{
...
void CUIEView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	else if (UIEMODE_EDIT == m_eMode)
	{
		...
		{	// m_RootUI�� �ڽ��߿��� point�� ��ġ�� ui �����ϱ�
			if(!(nFlags & MK_CONTROL)) pDoc->SetSelectedUI(NULL); // ��Ʈ�� Ű�� ������ ������ ��Ƽ ����Ʈ ������..
			...
			if(NULL == pUISelected && pRootUI->IsIn(point.x, point.y)) pUISelected = pRootUI; // �ϰ͵� �������� ��ƮUI�� ����.
...
BOOL CUIEView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	if (UIEMODE_EDIT == m_eMode && RT_NONE != m_eSelectedRectType)
	{
		...
		if (DRAGTYPE_NONE == m_eDragType)	// �巹�� ���� �ƴϸ� cursor�� ��ġ�� �� �׽�Ʈ�ϱ�
		{
			CPoint pt;
			if (GetCursorPos(&pt))
			{
		...
		case RT_REGION:
			{
				...
				for(int i = 0; i < iUIC; i++)
				{
					...
					else if(i == 0 && m_eDragType >= DRAGTYPE_LEFT && m_eDragType <= DRAGTYPE_RIGHTBOTTOM) // �������� ������ UI
					{
					...
					if(pSelectedUI->GetParent()) // �θ� UI �� ������..
					{
						pSelectedUI->GetParent()->ResizeAutomaticalyByChild(); // �ڵ����� ���� �ٽ� ���..
					}
				}
