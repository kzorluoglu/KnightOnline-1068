...
bool CTableGenerator::OpenSource(const std::string &szEnumFileName, const std::string &szTxtFileName)
{
	...
	{
		MessageBox(hWnd, szEnumFileName.c_str(), "������ ���ų� ���� �� �����ϴ�.", MB_OK);
		return false;
	}
	...
	{
		CloseHandle(hFile);
		MessageBox(hWnd, szEnumFileName.c_str(), "Data Type �� �����ϴ�.", MB_OK);
		return false;
	}
	...
	if (NULL == pFile)
	{
		MessageBox(hWnd, szTxtFileName.c_str(), "������ ���ų� ���� �� �����ϴ�.", MB_OK);
		return false;
	}
	...
	for(i = 0; true; i++)
	{
		...
		{
			int iSuccess = this->ParseLine(szLine, iOffset, iVal, dwVal, dfVal, szText);  // �����ϸ� 0, �߰��ؾ��ϰ� ���̸� -1 �߰��� �ʿ� ���� ���̸�.. -2 ����
			...
			if(iSuccess > 0)
			{
				...
				else
				{
					if(0 == j) // Key �ߺ� �˻�..
					{
						...
						{
							char szErr[512];
							sprintf(szErr, "Key �ߺ� : Line %d, Key : %d, File : %s", i + 1, iVal, szTxtFileName.c_str());
							MessageBox(hWnd, szErr, "Key �ߺ� - ���̺��� �߰� ����", MB_OK);
							break;
						}
					}
			...
			{
				MessageBox(hWnd, szTxtFileName.c_str(), "Data �� �� ������ ���ų� �ٸ��ϴ�.", MB_OK);
				fclose(pFile);
				m_Datas.clear();
				return false;
			}
...
bool CTableGenerator::OpenReference_Txt(int iIndex, const std::string &szTxtFileName)
{
	...
	for(int i = 0; true; i++)
	{
		...
		for(int j = 0; j < iDataCount; j++)
		{
			...
			{
				char szErr[512];
				sprintf(szErr, "Field ������ ���� �ʰų� ���ڿ��� �̻��մϴ� : Line %d, Field : %d", i + 1, j);
				MessageBox(hWnd, szErr, "���̺��� �߰� ����", MB_OK);
				fclose(pFile);
...
bool CTableGenerator::Generate(int iIndex, const std::string& szEnumFileName, const std::string &szTxtFileName)
{
	...
	{
		MessageBox(hWnd, "�⺻ ������ ���̺��� ���� ���̺��� �о�� �մϴ�.", "������ ���̺� ���� ����", MB_OK);
		return false;
	}
	...
	std::string szGrade[10] = { "", "����� ", "�ǹ� ", "��� ", "�÷�Ƽ�� ", "ũ���� ", "�糪 ", "�ֶ� ", "���μ�Ʈ ", "�̽�ƽ " };
	...
	std::vector<std::string>*	pLineArrays = new std::vector<std::string>[iCountWhole+1]; // ���� ���ۿ� �� ���ڿ� �迭 �غ�.. +1�� �ϴ� ������ ù�ٿ� ���� �������� �Ѵ�.
...
//						DWORD dwKey = ((m_Datas[j].m_dwValues[iIndexCur1] / 1000) * 1000) + m_DataExts[IG2_KEY].m_dwValues[iExt][iIndexCur2]; // ���ǰ� ���ڸ��� ��ġ�Ѵ�.
						DWORD dwKey = m_Datas[j].m_dwValues[iIndexCur1] + m_DataExts[IG2_KEY].m_dwValues[iExt][iIndexCur2]; // �����ش�.
				...
				case IG1_REQ_CHA:
					{
						bool bApplyAbility = false; // �䱸 �ɷ�ġ�� 0 �϶� �����ϰ�... ����� ���� 0 ���� ������ 0���� ǥ���Ұ�..
						...
						if(DT_STRING != dt) 
						{
							...
							{
								if(0 == m_Datas[j].m_iValues[iIndexCur1]) iValue = 0; // �䱸 �ɷ�ġ�� 0 �̸� ������� �ʴ´�.
...
//							lstrcpy(szBuff, "Invalid Percentage");
	...
	for(i = 0; i <= iCountWhole; i++) // �������� ����.
	{
		...
		for(int j = 0; j < iColCount; j++)
		{
			...
			if(j < iColCount - 1) WriteFile(hFile, "\t", 1, &dwRWC, NULL); // �� ����..
...
int CTableGenerator::ParseLine(const char* szLine, int& iOffset, int& iVal, DWORD& dwVal, double& dfVal, std::string& szText) // �����ϸ� 0, �߰��ؾ��ϰ� ���̸� -1 �߰��� �ʿ� ���� ���̸�.. -2 ����
{
...
bool CTableGenerator::Convert2Bin(const std::string& szFN)
{
	...
	if (NULL == stream)
	{
		MessageBox(hWnd, "���� �� ���� �����Դϴ�.", "Convert Error", MB_OK);
		return false;
	}
	...
	while(fgets(line, iMaxStrLen, stream))
	{
		...
		while(pcFind)
		{
			...
			{
				MessageBox(hWnd, "����ġ ���� ���� ����ǥ�� �ֽ��ϴ�.", "Convert Error", MB_OK);
				return false;
			}
		...
		if (!bQuotationActived) ++iCount;	// ����ǥ �����ִµ� �� �ٲ�� ī��Ʈ�� ���� �ʴ´�.
	}
	...
	{
		fclose(stream);
		std::string szMsg = szDestFN; szMsg += " - ������ ������ �� �����ϴ�.";
		MessageBox(hWnd, szMsg.c_str(), "Convert Error", MB_OK);
		return false;
	}
	...
	while(iRowCount<iCount)
	{
		...
		{
			char szErr[512];
			sprintf(szErr, "Key �ߺ� : Line %d, Key : %d, File : %s", iRowCount + 1, iKey, szFN.c_str());
			MessageBox(hWnd, szErr, "Key �ߺ� - ���̺��� �߰� ����", MB_OK);
			CloseHandle(hFile);
			fclose(stream);
			ASSERT(0);
		...
		while(iColCount<iDataCount)
		{
				...
				else
				{
					...
					{	// ����ǥ�� ������ ���ڿ� ���̴�.
						...
						if (FALSE == WriteData(hFile, m_DataTypes[iColCount], strValueBuffer.c_str()))
						{
							...
							MessageBox(hWnd, szErr, "����Ÿ ����� ���õǴ� ���� �ֽ��ϴ�. �� ������ ����� �۵� ���� ���� ���Դϴ�.", MB_OK);
							CloseHandle(hFile);
							fclose(stream);
							ASSERT(0);
							return false;
			...
			{
				int iSelect = MessageBox(hWnd, "���̺��� ���ڿ� �ܿ� �� ĭ�� �ֽ��ϴ�.\nYes : ����\nNo : �̹��� �����ϰ� ���\n Cancel : ��� �����ϰ� ���", "Convert Error", MB_YESNOCANCEL);
				if (IDYES == iSelect)
				{
					MessageBox(hWnd, "����Ÿ ����� �ߴܵǾ����ϴ�. �� ������ ����� �۵� ���� ���� ���Դϴ�.", "Convert Error", MB_OK);
					CloseHandle(hFile);
					fclose(stream);
					ASSERT(0);
					return false;
		...
		if (bQuotationActived) continue;	// ����ǥ�� ���� ������ ��� ���ڿ��̹Ƿ� �ٽ� ����..
		if (iDataCount != iColCount)
		{
			char szErr[512];
			wsprintf(szErr, "File - %s, Line - %d, Field - %d", szFN.c_str(), iRowCount + 1, iColCount);
			MessageBox(hWnd, szErr, "���� ������ ����Ÿ �׸�� ��ϵǴ� ����Ÿ �׸��� ��ġ���� �ʴ� �׸��� �ֽ��ϴ�.", MB_OK);
			CloseHandle(hFile);
			fclose(stream);
			ASSERT(0);
			return false;
	...
	if(INVALID_HANDLE_VALUE == hFile)
	{
		MessageBox(hWnd, szDestFN, "��ȣȭ ���� ����⿡ ���� �߽��ϴ�.", MB_OK);
		remove(szDestFN); // ���� �����..
		return false;
	}
...
//}
	...
	{
		char szErr[512];
		wsprintf(szErr, "%d���� ����Ÿ ����� �Ϸ�Ǿ����ϴ�.", iRowCount);
	...
	{
		char szErr[512];
		wsprintf(szErr, "��ü %d���� ����Ÿ�� %d���� ����Ÿ�� ����� �Ϸ�Ǿ����ϴ�.\n�� ���̺��� ����� �۵����� �������Դϴ�.", iCount, iRowCount);
		MessageBox(hWnd, szErr, "����Ÿ ���� ����", MB_OK);
		return false;
	}
}
