...
BOOL CLauncherDlg::OnInitDialog()
{
	...
	m_progress.SetColor(RGB(64, 255, 64)); // ???????? ???? ?????.
	...
	// Read PATH
	GetPrivateProfileString("Config", "PATH", ".\\", szBuff, 256, szIniPath);
...
void CLauncherDlg::PacketReceive_Version(const BYTE *pBuf, int &iIndex)
{
	...
	else if( m_nCurVersion < m_nServerVersion ) // ?????? ??????..
	{
		PacketSend_DownloadInfo(); // ????? ???..
	}
	else 
	{
...
void CLauncherDlg::DownloadProcess()
{
	...
	for( int i=0; i<m_nGetFileNum; i++ )
	{
		...
		{
			CString szErr; szErr.LoadString(IDS_ERR_DOWNLOAD_PATCH_FILE_AND_RETRY); // ??? ????????
...
//	WritePrivateProfileString("VERSION","CURRENT",version, inipath);
	...
	if(true == bExtractSuccess && m_hRegistryKey) // ???? ???? ????, ???? ???? ?????? ??????? ?????? ????..
	{
			...
			case FD_READ:
				{
					...
					while ( m_pSocket->m_qRecvPkt.size() > 0 )			// ??? ??????? ????? ???????
					{
						int iOffset = 0;
						DataPack* pDataPack = m_pSocket->m_qRecvPkt.front();	// ??? ???? ???? ????..
						this->PacketProcess(pDataPack->m_pData, iOffset);		// ????? ????? ????? ????.
						delete pDataPack;
						m_pSocket->m_qRecvPkt.pop();							// ????? ????? ????..
					}
				}
				break;
			default:
