...
void CDlgBase::UpdateInfo()
{
	...
	if(pBase->Type() & OBJ_SHAPE)
	{
		...
		pItem = m_LPShape.GetPropItem("�Ҽ�");			if(pItem) pItem->m_curValue.Format("%d", pSI->m_iBelong);
		...
		pItem = m_LPShape.GetPropItem("NPC Status");		if(pItem) pItem->m_curValue.Format("%d", pSI->m_iNPC_Status); // NPC �� ���� ������Ʈ�� ��� NPC Type
...
BOOL CDlgBase::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	if(pBase != NULL)
	{
		...
		if((void*)wParam == &m_LPShape && (pBase->Type() & OBJ_SHAPE))
		{
			...
			if(pItem->m_propName == "�Ҽ�" && pItem->m_curValue.GetLength() > 0)			pSI->m_iBelong = atoi(pItem->m_curValue);
			...
			else if(pItem->m_propName == "NPC Status" && pItem->m_curValue.GetLength() > 0) pSI->m_iNPC_Status = atoi(pItem->m_curValue); // NPC �� ���� ������Ʈ�� ��� NPC Type
