...
void CPondMesh::Release()
{
	m_iPondID = -1;
	m_fWaterHeight = 0.0f;	//	������
	m_iWaterScaleWidth = 4;	//	���ι����� ���� ����	
	m_iWaterScaleHeight = 6;//	���ι����� ���� ����	
	m_iBackUpWidht = 0;
	m_iBackUpHeight = 0;
	m_dwPondAlpha = 0x88ffffff;		//	���� ����	
	m_fTU = 50.0f;	
	m_fTV = 50.0f;	
...
void CPondMesh::Render()
{
	...
	if (m_pTexture && m_iVC>2  && m_iIC>0)
	{
		...
		if ((m_dwPondAlpha & 0xff000000) != 0xff000000)	// alpha factor �����ϱ�
		{
			// render state ����
			...
			s_lpD3DDev->SetRenderState(D3DRS_TEXTUREFACTOR, m_dwPondAlpha);	// alpha factor ����
			// texture state ����(alpha)
...
void CPondMesh::RenderVertexPoint()	// �ߺ��̰� ���� �ٽ� �׸���
{
...
void CPondMesh::SetWaterHeight(float fHeight)
{
	...
	MakeDrawRect(m_vDrawBox);	//	�ܰ��� �ٽ� ����
	UpdateWaterHeight();	//	�������� ������ ���̷� �ø�
}
...
void CPondMesh::MakeDrawRect(__Vector3* p4vPos)	//	ȭ�鿡 ���� ������ �����׵θ� �����
{
	if(p4vPos==NULL) return;

	memcpy(m_vDrawBox,p4vPos,sizeof(__Vector3)*4);
	m_fWaterHeight = p4vPos[0].y;	//	���� ���̸� �޴´�
	p4vPos = NULL;	//	�޸𸮿��� �̻��� ����� ���ñ��..

	//----------------------------------------------------------------------------
	DWORD color = 0xffffff00;
	m_ViewRect[0].Set(m_vDrawBox[0],color);	//	����
	m_ViewRect[1].Set(m_vDrawBox[1],color);	//	����
	m_ViewRect[2].Set(m_vDrawBox[2],color);	//	�Ʒ���
	m_ViewRect[3].Set(m_vDrawBox[3],color);	//	�Ʒ���
...
float CPondMesh::SettingDrawPos(__Vector3 vPos1,__Vector3 vPos2,__Vector3 vPos3,__Vector3 vPos4,int iLinePosNum)
{
	...
	LinePos = vPos2-vPos1;
	fLength = LinePos.Magnitude(); fLength /= (iLinePosNum+1);	//	���̱��ϰ�,�������� ������ ��ġ�� ���� ���
	LinePos.Normalize(); LinePos *= fLength;	//	����1m�� ���͸� ���ϰ�,������ �Ÿ��� �Է�
...
void CPondMesh::MakePondPos()
{
	...
	ASSERT(MAX_PONDMESH_VERTEX>m_iWaterScaleWidth*m_iWaterScaleHeight);	//	������ ������� ���Ǽ��ڰ� max�� ���ϴ��� Ȯ��
	int ix,iz,itemp;
...
void CPondMesh::MakeIndex()
{
	...
	int j,k;
	int m = m_iWaterScaleWidth;	//	������
	int x=0,y=m;
	WORD* indexPtr = m_pdwIndex;	//	�ﰢ���� �θ� ��ġ ����
...
{
	__Vector3		pBakVertices[MAX_PONDMESH_VERTEX];	//	����,��������� ���̴� ��
	__VertexXyzT2	pBakViewVts[MAX_PONDMESH_VERTEX];	//	ȭ�鿡 �ѷ����� ��
...
BOOL CPondMesh::InputDummyMovingPos(__Vector3 vDummyMovingPos,BOOL bMovePond)
{
	...
	if(vDummyMovingPos.y!=0)
		SetWaterHeight(m_vSelectBox[0].y);	//	���̸� üũ

	//	���� ������ ó��
	if(bMovePond)
	{		
		for(int i=0;i<4;++i) m_vDrawBox[i] += vDummyMovingPos;
		MakeDrawRect(m_vDrawBox);	//	�����ڸ� ���� �� ���
		UpdateMovePos(vDummyMovingPos);	//	����� ��� ���� �� ���
	}
	else
	{
...
void CPondMesh::CalcuWidth(int iSx,int iSy,int iEx,int iEy)
{
	...
	//	----------------------------------------------------------------------------------
	//	����ؾ��� ù���� ���Ѵ�
	__Vector3 vBakPick = *(m_pVertices + iHeightNum + iSx);	//	����� ���� ��ǥ(�����Ͽ� ���ο� ��ǥ ���)
	__Vector3 vNowPick = *(m_pViewVts + iHeightNum + iSx);	//	���� ������ ��(���߿� ������ �����ǰ�)
	__Vector3* pLRVertices = m_pVertices + iHeightNum;		//	����� ù ��(�����)
	...
	if(iIntervalNum>0)
	{
		...
		for(j=0 ; j<iIntervalNum ; ++j,++pLRVertices,++pLRViewVer,++pvTop,++pvBottom)
		{
			...
			if(fx2!=0)
			{
				...
				fnx1 = fnx2*ftemp;
				pLRViewVer->x = pvRight->x - fnx1;	//	x�� ���ο��� �Է�

				//	��������x-�����x : ��������x-�������ϴ���x =  ���ο���z-�����z : ���Һ�ȭ��z
				fny1 = fny2*ftemp;
				pLRViewVer->z += fny1;				//	z�� ��ȭ�� ����
			}
		}
	}
	//	----------------------------------------------------------------------------------
	...
	//	----------------------------------------------------------------------------------
	iIntervalNum = m_iWaterScaleWidth - iSx-2;	//	�¿� �� ������ ���⿡
	if(iIntervalNum>0)
	{
		++pLRVertices, ++pLRViewVer;	//	����
		fny2 = vNowPick.z-vBakPick.z;
		for(j=0 ; j<iIntervalNum ; ++j,++pLRVertices,++pLRViewVer,++pvTop,++pvBottom)
		{
			...
			if(fx2!=0)
			{
				...
				fnx1 = fnx2*ftemp;
				pLRViewVer->x = pvLeft->x + fnx1;		//	x�� ���ο��� �Է�
...
void CPondMesh::SetAllPos(int iSx,int iSy,int iEx,int iEy)
{
	...
	vCenter = *(m_pVertices + m_iWaterScaleWidth*iSy + iSx);	//	���� ������ ��(���߿� ������ �����ǰ�)
	vNowCenter = *(m_pViewVts + m_iWaterScaleWidth*iSy + iSx);	//	����� ���� ��ǥ(�����Ͽ� ���ο� ��ǥ ���)
	...
	if(iIntervalNum>0)
	{
		...
		if(fy2!=0)
		{
			...
			{
				fy1 = pTBVertices->z - vTop.z; 	//	�¿��� ��������
				ftemp = fy1/fy2;

				fnx1 = fnx2*ftemp;
				pTBViewVer->x += fnx1;			//	x�� ��ȭ�� ����

				fny1 = fny2*ftemp;
				pTBViewVer->z = vTop.z - fny1;	//	z�� ���ο��� �Է�
			}
		}
	}
	...
	if(iIntervalNum>0)
	{
		...
		if(fy2!=0)
		{
			...
			for(j=0 ; j<iIntervalNum ; ++j,pTBVertices+=m_iWaterScaleWidth,pTBViewVer+=m_iWaterScaleWidth)
			{
				...
				fny1 = fny2*ftemp;
				pTBViewVer->z = vBottom.z + fny1;	//	z�� ���ο��� �Է�
			}
...
bool CPondMesh::Load1001(HANDLE hFile)
{
	...
	ReadFile(hFile, &m_iPondID, sizeof(m_iPondID), &dwNum, NULL);		// ���� ��ȣ
	ReadFile(hFile, &m_dwPondAlpha, sizeof(m_dwPondAlpha), &dwNum, NULL);		// ���� ����
	...
	ReadFile(hFile, m_vDrawBox, sizeof(m_vDrawBox), &dwNum, NULL);				// ���ٿ� �ִ� �� ����

	ReadFile(hFile, &m_iVC, sizeof(m_iVC), &dwNum, NULL);				// �� ����
	if (m_iVC>0)
	{
		ReadFile(hFile, m_pViewVts, m_iVC*sizeof(__VertexXyzT2), &dwNum, NULL);	// vertex buffer
		ReInputBackPos();	//	����뿡 ������ǥ�Է�
	}
...
bool CPondMesh::Load1000(HANDLE hFile)
{
	...
	ReadFile(hFile, &m_iWaterScaleWidth, sizeof(m_iWaterScaleWidth), &dwNum, NULL);	// ���ٿ� �ִ� �� ����
...
bool CPondMesh::Load(HANDLE hFile)
{
	...
	ReadFile(hFile, &m_iWaterScaleWidth, sizeof(int), &dwNum, NULL);				// ���ٿ� �ִ� �� ����
...
bool CPondMesh::Save(HANDLE hFile)
{
	...
	WriteFile(hFile, &m_iPondID, sizeof(m_iPondID), &dwNum, NULL);		// ���� ��ȣ
	WriteFile(hFile, &m_dwPondAlpha, sizeof(m_dwPondAlpha), &dwNum, NULL);		// ���� ����
	...
	WriteFile(hFile, m_vDrawBox, sizeof(m_vDrawBox), &dwNum, NULL);				// ���ٿ� �ִ� �� ����

	WriteFile(hFile, &m_iVC, sizeof(m_iVC), &dwNum, NULL);				// �� ����
...
//	vStPos = pViewVts[(m_iVC+m_iLastVertexNum)/2].v;	//	���� ������ �߰� ��ġ����
	vStPos = vCenter;
...
//	ASSERT(m_iVC<2);
	ASSERT(MAX_PONDMESH_VERTEX>m_iVC+m_iWaterScaleWidth*m_iWaterScaleHeight);	//	������ ������� ���Ǽ��ڰ� max�� ���ϴ��� Ȯ��
