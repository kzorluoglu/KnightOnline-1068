...
BOOL CDlgPondProperty::OnInitDialog() 
{
	...
	m_LPPond.AddPropItem("Texture File", "", PIT_FILE, "Texture �� ���� �ִ� �׸� ����(*.DXT; *.BMP; *.TGA)|*.DXT; *.BMP; *.TGA|");
...
//	m_LPPond.ShowWindow(TRUE);
	if (m_IsModalDialog==FALSE)
	{
		...
		if(pSelPond->GetChangUVState()==TRUE)	
			 GetDlgItem(IDC_BU_RECALCUV)->SetWindowText("������");
		else GetDlgItem(IDC_BU_RECALCUV)->SetWindowText("�����ϰ�");
...
BOOL CDlgPondProperty::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	if((void*)wParam == &m_LPPond)
	{
		if (pSelPond)
		{
			...
			{
				CN3Base tmp;
				tmp.m_szName = pItem->m_curValue;		// ����η� �ٲٱ�
...
void CDlgPondProperty::OnButtonRecalUV() 
{
	...
	{
		GetDlgItem(IDC_BU_RECALCUV)->SetWindowText("�����ϰ�");
...
void CDlgPondProperty::OnButtonDeletePond() 
{
	...
	if (pSelPond)
	{
		if (MessageBox("���õ� ������ ����ðڽ��ϱ�?", "Remove pond", MB_YESNO|MB_DEFBUTTON2) == IDNO) return;
