...
/*	if(bGenerateChainNumber)
	{
		...
		for(int i = 0; i < nSC; i++)
		{
			...
			szCompare[nL-5] = NULL; // �ڿ� �ٴ� ����ٿ� ���ڸ� ��ȣ�� ����..
			if(0 == lstrcmpi(pShape->Name(), szCompare)) // �̸��� ������..
			{
				nChainNumber = atoi(&(szCompare[nL-4])) + 1;
			}
		}
		...
		pShape->FileNameSet(szFileName2); // ���� �̸� ����...
	}
*/
...
void CMapMng::SavePartition(float x, float z, float width)
{
	...
	CFileDialog dlg(FALSE, "n3m", NULL, dwFlags, "N3ME ����(*.n3m)|*.n3m||", NULL);
...
void CMapMng::Render()
{
	if(false == m_bLoadingComplete) return; // �ε��� ���� �� ������...
	...
	if (m_bRenderAxisAndGrid) // ��� �׸��� �׸���...
	{
		...
		if(m_SelOutputObjArray.GetSize() > 0)
		{
			...
			if(pSelObj) // ���õ� ��ü�� ������..
			{
				mtxWorld.PosSet(0, pSelObj->Pos().y, 0); // ���̸� �ø���.
			}
		}
		pEng->RenderGrid(mtxWorld); // �׸��� �׸���...
	}
	...
	for (i=0; i<iSize; ++i)
	{
		...
		if(pSelObj->Type() & OBJ_SHAPE)
		{
			...
			((CN3Shape*)pSelObj)->RenderCollisionMesh(); // �浹 �޽� �׸���...
			((CN3Shape*)pSelObj)->RenderClimbMesh(); // �ö󰡴� �޽� �׸���..
		}
	}
...
BOOL CMapMng::CameraMove(LPMSG pMsg)
{
	...
	else if (iButtonDownCount == 0 && !(GetAsyncKeyState(VK_MENU) & 0xff00) ) return FALSE;	// alt Ű�� �ȴ��������� ī�޶� �������� �ƴϴ�.
...
BOOL CMapMng::MouseMsgFilter(LPMSG pMsg)
{
	...
	if (bSelectDrag == FALSE &&							// Object drag select ��尡 �ƴϰ�
		RCM_SELECT != m_RiverMng.GetRCursorMode())		// �������� drag select��尡 �ƴҶ�
	{
		CN3Camera* pCamera = m_pSceneOutput->CameraGetActive();
		if(pCamera && pCamera->MoveByWindowMessage(pMsg)) return TRUE; // ī�޶� �̵� �޼��� �Ÿ���
	}
	...
	if (m_pTerrain && m_pTerrain->MouseMsgFilter(pMsg))	return TRUE;// ���� ���� �޼��� �Ÿ���
	if (m_RiverMng.MouseMsgFilter(pMsg)) return TRUE;				// �� ���� �޼��� �Ÿ���
	if (m_PondMng.MouseMsgFilter(pMsg)) return TRUE;				// ���� ���� �޽��� �Ÿ��� 
	if (m_pNPCPath && m_pNPCPath->MouseMsgFilter(pMsg)) return TRUE;// NPC �游��� �޼��� �Ÿ���
	...
	case WM_LBUTTONUP:	// ��ü ����
		{
			...
			{
				POINT point = {short(LOWORD(pMsg->lParam)), short(HIWORD(pMsg->lParam))};
				if (abs(m_rcSelDrag.left-point.x)<4 && abs(m_rcSelDrag.top-point.y) < 4) // Ŭ���Ѱɷ� ����
				{
					int nPart = -1;
					CN3Base* pBaseSel = Pick(point, &nPart);
										
					SelectObject(pBaseSel, FALSE, (GetAsyncKeyState(VK_SHIFT) & 0xff00) ? TRUE : FALSE );
					if(m_pDlgBase && pBaseSel && pBaseSel->Type() & OBJ_SHAPE) // ��Ʈ ����..
					{
				...
				m_pMainFrm->UpdateTransformInfo(); // ��ġ, ȸ�������� ������Ʈ�Ѵ�.
...
void CMapMng::SelectObject(CN3Base* pObj, BOOL IsSourceObj, BOOL bAdd)
{
	...
	{	// �̹� ��ġ�� ��ü ����
		...
		if (pObj && pObj->Type() & (OBJ_CHARACTER | OBJ_SHAPE))
		{
			...
			if (bAdd)	// �߰�
			{
				...
				if (bAleadySelected) m_SelOutputObjArray.RemoveAt(i);	// �̹� �����Ƿ� ���ø�Ͽ��� ����
				else m_SelOutputObjArray.InsertAt(0, pSelObj);			// �߰�
			}
			else	// ���� ����
			{
				m_SelOutputObjArray.RemoveAll();
	...
	if(m_SelOutputObjArray.GetSize() == 1) // �Ѱ��� �����ߴٸ�..
	{
	...
	for (i=0; i<iSC;)
	{
		...
		if (fScreenZ<1.0 && fScreenZ>0.0)
		{
				...
				fScreenY >= pRect->top && fScreenY <= pRect->bottom)
			{
				...
				if (bAleadySelected) m_SelOutputObjArray.RemoveAt(j);	// �̹� �����Ƿ� ���ø�Ͽ��� ����
				else m_SelOutputObjArray.InsertAt(0, pObj);			// �߰�
			}
		}
		++i;
	}
...
void CMapMng::MakeGameFiles(LPCTSTR lpszPathName, float fSize)
{
	...
	{
		m_pTerrain->m_szName = szFName; // �̸��� �����Ѵ�.. �� �̸���� ����ȴ�.
	...
	// Shape Manager ����� ����...
//	if(true == m_pSceneOutput->CheckOverlappedShapesAndReport()) // �̸��̳� ��ġ �ߺ� Ȯ��..
//	{
//		int idYesNo = ::MessageBox(::GetActiveWindow(), "�ߺ��� ������Ʈ���� ���� �Ͻðڽ��ϱ�?", "�ߺ��� ������Ʈ ó��", MB_YESNO);
//		if(IDYES == idYesNo)
//		{
//			m_pSceneOutput->DeleteOverlappedShapes();
//			m_pDlgOutputList->UpdateTree(m_pSceneOutput); // �ƿ�ǲ ����Ʈ üũ..
//			m_SelOutputObjArray.RemoveAll(); // ������ �ʱ�ȭ..
//		}
//	}
	...
	for(int i = 0; i < nSC; i++)
	{
		ShapeMgr.Add(m_pSceneOutput->ShapeGet(i)); // Shape �߰�.
	}

	if(m_pWall) 
		m_pWall->AddWall2Coll(&ShapeMgr);
	ShapeMgr.GenerateCollisionData(); // �浹 �޽� �����͸� �����Ѵ�...
	char szObjPosting[_MAX_PATH] = "";
	_makepath(szObjPosting, szDrive, szDir, szFName, ".opd");  // "Object Posting Data" - Shape Manager file �̸��� ���ϰ�..
...
{
	m_pTerrain->MakeMoveTable(m_pEventMgr->m_ppEvent); //������ �Ӽ� ����...
	pShapeMgr->MakeMoveTable(m_pEventMgr->m_ppEvent);	//������ �Ӽ� ����...
}
...
//	}
	...
	ShapeMgr.SaveCollisionData(hFile); // �浹 �����͸� ����...
	...
	for(i = 0; i < nSC; i++)
	{
		...
		if(pShape->m_iEventID || pShape->m_iEventType || pShape->m_iNPC_ID || pShape->m_iNPC_Status ) // �̺�Ʈ�� ������
			iEventObjectCount++;
	}
	...
	*/
	...
	m_pEventMgr->SaveInfoTextFile(szEvent);	//�������� ���� �̺�Ʈ ���̵� ���ǵ��� ��� �ִ� �ؽ�Ʈ ����..
...
/*	
	...
	char szDrive[_MAX_DRIVE], szDir[_MAX_DIR], szFName[_MAX_FNAME], szExt[_MAX_EXT];
	_splitpath(lpszPathName, szDrive, szDir, szFName, szExt);
	_makepath(szCollisionFN, szDrive,szDir, szFName, ".scd"); // �ٸ� �̸����� ����..
	CFile file;
	file.Open(szCollisionFN, CFile::modeCreate | CFile::modeWrite);
	ShapeMgr.SaveCollisionData((HANDLE)file.m_hFile); // �浹 �����͸� ����...
	file.Close();
*/
	CloseHandle(hFile);
}
...
//	pEng->s_CameraData.vEye = vEye;	// Shape�� LOD���� ������..
...
void CMapMng::SetCursorMode(int iMode)
{
	...
	m_pBrushDlg->SetTerrain(NULL);			// Brush â�� ������ �ʰ��Ѵ�.
	...
	if (m_pDummy)
	{
		...
		m_pDummy->m_pTerrainRef = m_pTerrain; // ���̿� ���� �����͸� �־��ش�.
	}
...
void CMapMng::RenderGrid(float fGridSize, float fMaxDistance)	// fGridSizeũ��� fMaxDistance�Ÿ����� ���ڸ� �׷��ش�
{
...
/*
	...
	for(i=0; i<iSC; ++i)
	{
		...
		pShape->SaveToFile(szSFN); // Shape ���� binary file�� ����..

		fprintf(stream, "%s\n", pShape->Name()); // �ؽ�Ʈ�� Shape ���� �̸��� ����..
	}
	fclose(stream);
	// OldData
	//////////////////////////////////
*/
	...
	// ���� �̸��� �и��ϰ�..
	char szDrive[_MAX_DRIVE], szDir[_MAX_DIR], szFName[_MAX_FNAME], szExt[_MAX_EXT];
	_splitpath(lpszFileName, szDrive, szDir, szFName, szExt); // ���� �̸��� Ȯ���ڸ� ����..
	...
	for(i=0; i<iSC; ++i)
	{
		...
		_splitpath(pShape->FileName().c_str(), NULL, NULL, szFName, szExt); // ���� �̸��� Ȯ���ڸ� ����..
		_makepath(szSFN, NULL, NULL, szFName, szExt); // ���� �̸��� �ٽ� �����.
...
void CMapMng::LoadObjectPostData(LPCTSTR lpszFileName)
{
	...
	if (stream == NULL) {m_pMainFrm->MessageBox("������ �ؽ�Ʈ ������ ã�� �� �����ϴ�."); return;}
	...
	fgets(szFirstLine, 256, stream); // ù° ���� �а�..
	if(strstr(szFirstLine, "Shape Count : ")) // ���ڿ��� ������ ���� �������̴�..
	{
		int iSC = 0, result = 0;
		...
		for(int i=0; i < iSC; ++i)
		{
			...
			if(false == pShape->LoadFromFile(szSFN))
			{
				...
				continue; // Shape ���� binary file�� �б�..
			}

			szDestName[lstrlen(szDestName) - 5] = NULL; // _0000 ���ڿ��� ����..
			_makepath(szSFN, NULL, NULL, szDestName, ".n3shape");
			pShape->m_szName = szDestName;
			pShape->FileNameSet(std::string(szSFN)); // �ٽ� ���� �̸� ����..
	...
	else // ���� ���� �������̴�..
	{
		...
		{
			CN3Shape* pShape = new CN3Shape();
			m_pSceneOutput->ShapeAdd(pShape); // �߰�..
			...
			pShape->LoadFromFile(szSFN2); // ���Ͽ��� �а�..
			for(int j = 0; j < iSPC; j++)
			{
				fgets(szLine, 1024, stream);
...
void CMapMng::ImportPostDataFromScene(const char *szFileName)
{
	...
	int nCC = 0;
	ReadFile(hFile, &nCC, 4, &dwRWC, NULL); // ī�޶�..
	for(i = 0; i < nCC; i++)
	{
	...
	int nLC = 0;
	ReadFile(hFile, &nLC, 4, &dwRWC, NULL); // ī�޶�..
	for(i = 0; i < nLC; i++) 
	{
		ReadFile(hFile, &nL, 4, &dwRWC, NULL);
		if(nL <= 0) continue;
	...
	int nChrC = 0;
	ReadFile(hFile, &nChrC, 4, &dwRWC, NULL); // ĳ����
	for(i = 0; i < nChrC; i++)
	{
		ReadFile(hFile, &nL, 4, &dwRWC, NULL);
		if(nL <= 0) continue;
	...
	m_pDlgOutputList->UpdateTree(m_pSceneOutput); // Ʈ�� ������Ʈ...
...
void CMapMng::DeleteUnusedFiles()
{
	...
	// ��� ������Ʈ..
	m_pSelSourceObj = NULL; // �̷��� ���־�� ���� �ȳ���.
	m_SelOutputObjArray.RemoveAll();
	this->LoadSourceObjects(); // Source Object �� �ٽ� �а�..
	this->UpdateAll(); // ���� ������Ʈ...
}

void CMapMng::DeleteOverlappedObjects() // ��ġ�� ��ģ ��Ʈ�� ã�´�.
{
	...
	for(i = 0; i < iSC; i++)
	{
		m_pSceneOutput->ShapeDelete(OverlappedObjects[i]); // ��ģ�� �����..
	}

	// ������Ʈ...
	m_pSelSourceObj = NULL; // �̷��� ���־�� ���� �ȳ���.
	m_SelOutputObjArray.RemoveAll();
	if (m_pDlgOutputList) m_pDlgOutputList->UpdateTree(m_pSceneOutput); // ���� ������Ʈ...
}
...
void CMapMng::DeleteSelectedSourceObjects()
{
	...
	for(i = 0; i < iSC; i++)
	{
		m_pSceneOutput->ShapeDelete(SameObjects[i]); // ��ģ�� �����..
	}
	...
	case eEDIT_PASTE:
		{
			if(m_eSelObjBackState == eEDIT_COPY)
			{
				...
				m_SelOutputObjArray.RemoveAll();	//	���� ���õ� ������ �����
				for(j=0;j<iSize;++j)
				{
					pDestObj = m_SelOutputObjBack.GetAt(j);	//	����� �����͸� ã�´�
					if(pDestObj==NULL) continue;

					pNewObj = AddObjectToOutputScene(pDestObj);	//	�ּ��� �����θ� ���ο� �����͸� �����
					if(pNewObj == NULL) continue;

					m_SelOutputObjArray.InsertAt(0,pNewObj);	//	���� ������ ���� �Է��Ѵ�

					pNewObj->ScaleSet(pDestObj->Scale());	//	ũ�� �Է�
					pNewObj->RotSet(pDestObj->Rot());	//	ȸ���� �Է�

					vObjPos = pDestObj->Pos() + vNewPos;
					vObjPos.y = m_pTerrain->GetHeight(vObjPos.x,vObjPos.z);
					pNewObj->PosSet(vObjPos);		//	���ο� ��ġ�� �Է�
				}
				OnSelChanged();	//	�������Էµ� ���ο���ͷ� ����
			}
		}
