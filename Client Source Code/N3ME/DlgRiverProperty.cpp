...
BOOL CDlgRiverProperty::OnInitDialog() 
{
	...
	m_LPRiver.AddPropItem("Texture File", "", PIT_FILE, "Texture �� ���� �ִ� �׸� ����(*.DXT; *.BMP; *.TGA)|*.DXT; *.BMP; *.TGA|");
	...
	m_LPRiver.AddPropItem("Animation Texture File", "", PIT_FILE, "Texture �� ���� �ִ� �׸� ����(*.DXT; *.BMP; *.TGA)|*.DXT; *.BMP; *.TGA|");
...
BOOL CDlgRiverProperty::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	...
	if((void*)wParam == &m_LPRiver)
	{
		if (pSelRiver)
		{
			...
			{
				CN3Base tmp;	tmp.m_szName = pItem->m_curValue;		// ����η� �ٲٱ�
			...
			else if (pItem->m_propName == "Animation Texture File")
			{
				...
				int iCount = atoi(szFName+lstrlen(szFName)-2) + 1;	// ���� �̸��� ���� ���ڸ��� ���ڷ� ��ȯ
...
void CDlgRiverProperty::OnOK() 
{
	if (m_IsModalDialog)
	{
		...
		if (pSelRiver && pSelRiver->TexGet())
		{
			...
			else MessageBox("�ߺ��Ǵ� ���̵� �Դϴ�.");
		}
		else
		{
			MessageBox("Texture�� �������� �ʾҽ��ϴ�.");
		}
	}
}
...
void CDlgRiverProperty::OnButtonDeleteRiver() 
{
	...
	if (pSelRiver)
	{
		if (MessageBox("���õ� ���� ����ðڽ��ϱ�?", "Remove river", MB_YESNO|MB_DEFBUTTON2) == IDNO) return;
