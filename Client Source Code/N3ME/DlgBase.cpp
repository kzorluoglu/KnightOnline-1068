...
BOOL CDlgBase::OnInitDialog() 
{
	...
	m_LPCamera.AddPropItem("�Ȱ� ���", "", PIT_EDIT, "");
//	m_LPCamera.AddPropItem("�Ȱ� �е�", "", PIT_EDIT, "");
	m_LPCamera.AddPropItem("�Ȱ� ��", "", PIT_EDIT, "");
//	m_LPCamera.AddPropItem("�Ȱ� ����", "", PIT_EDIT, "");
//	m_LPCamera.AddPropItem("�Ȱ� ��", "", PIT_EDIT, "");
...
//	m_CBShapePart.SetCurSel(0);
	...
	m_LPShape.AddPropItem("�Ҽ�", "", PIT_EDIT, "");
	...
	m_LPShape.AddPropItem("Part Add", "Part �߰�", PIT_BUTTON, "");
	m_LPShape.AddPropItem("Part Delete", "Part ����", PIT_BUTTON, "");

	m_LPShape.AddPropItem("Face, Vertex Count", "", PIT_EDIT, "");
	m_LPShape.AddPropItem("Texture File", "", PIT_FILE_MULTI, "Texture �� ���� �ִ� �׸� ����(*.DXT; *.BMP; *.TGA)|*.DXT; *.BMP; *.TGA|");
	...
	m_LPShape.AddPropItem("Collision Mesh Delete", "Collision Mesh ����", PIT_BUTTON, "");
	...
	m_LPChr.AddPropItem("Collision Mesh Delete", "Collision Mesh ����", PIT_BUTTON, "");
//	m_LPChr.AddPropItem("Collision Skin File", "", PIT_FILE, "N3 Skin File(*.N3Skin)|*.N3Skin||");
//	m_LPChr.AddPropItem("Collision Skin Delete", "Collision Skin ����", PIT_BUTTON, "");

	m_LPChr.AddPropItem("Part Add", "Part �߰�", PIT_BUTTON, "");
	m_LPChr.AddPropItem("Part Delete", "Part ����", PIT_BUTTON, "");
	m_LPChr.AddPropItem("Plug Add", "Plug �߰�", PIT_BUTTON, "");
	m_LPChr.AddPropItem("Plug Delete", "Plug ����", PIT_BUTTON, "");
	...
	m_LPCPart.AddPropItem("Part Type", "", PIT_COMBO, "�Ӹ�ī��|��|��ü|��ü|��|��|??|");
	m_LPCPart.AddPropItem("Texture File", "", PIT_FILE, "Texture �� ���� �ִ� �׸� ����(*.DXT; *.BMP; *.TGA)|*.DXT; *.BMP; *.TGA|");
...
//	m_LPCPart.AddPropItem("Skin File", "", PIT_FILE, "N3 Skin File(*.N3Skin)|*.N3Skin||");
	...
	m_LPCPlug.AddPropItem("Plug Type", "", PIT_COMBO, "����������|�޼�����|�������");
	...
	m_LPCPlug.AddPropItem("Plug Texture File", "", PIT_FILE, "Texture �� ���� �ִ� �׸� ����(*.DXT; *.BMP; *.TGA)|*.DXT; *.BMP; *.TGA|");
...
//		if(pItem) pItem->VectorSet(pT->Rot());
	...
	if(pBase->Type() & OBJ_CAMERA)
	{
		...
		pItem = m_LPCamera.GetPropItem("�Ȱ� ���");
		if(pItem)
		{
			if(pC->m_bFogUse) pItem->m_curValue = "On";
			else pItem->m_curValue = "Off";
		}
//		pItem = m_LPCamera.GetPropItem("�Ȱ� �е�");
...
//		if(pItem) pItem->m_curValue.Format("%f", pC->m_fFogEnd);
	...
	if(pBase->Type() & OBJ_SHAPE)
	{
		...
		pItem = m_LPShape.GetPropItem("�Ҽ�");			if(pItem) pItem->m_curValue.Format("%d", pS->m_iBelong);
		...
		pItem = m_LPShape.GetPropItem("NPC Status");		if(pItem) pItem->m_curValue.Format("%d", pS->m_iNPC_Status); // NPC �� ���� ������Ʈ�� ��� NPC Type
...
//			else if(pItem->m_propName == "�Ȱ� �е�") pC->m_fFogDensity = (float)atof(pItem->m_curValue);
			else if(pItem->m_propName == "�Ȱ� ��") pC->m_FogColor = pItem->D3DColorGet();
//			else if(pItem->m_propName == "�Ȱ� ����") pC->m_fFogStart = (float)atof(pItem->m_curValue);
//			else if(pItem->m_propName == "�Ȱ� ��") pC->m_fFogEnd = (float)atof(pItem->m_curValue);
		}
		...
		if((void*)wParam == &m_LPShape && (pBase->Type() & OBJ_SHAPE))
		{
			...
			if(pItem->m_propName == "�Ҽ�" && pItem->m_curValue.GetLength() > 0)			pS->m_iBelong = atoi(pItem->m_curValue);
			...
			else if(pItem->m_propName == "NPC Status" && pItem->m_curValue.GetLength() > 0) pS->m_iNPC_Status = atoi(pItem->m_curValue); // NPC �� ���� ������Ʈ�� ��� NPC Type
		...
		if((void*)wParam == &m_LPCPart && (pBase->Type() & OBJ_CHARACTER))
		{
			...
			if(nPart >= 0 && nPart < nPartCount)
			{
				...
				if(pItem->m_propName == "Part Type")
				{
					...
					if(pPD)
					{
//del m_Type			if(pItem->m_curValue == "�Ӹ�ī��") pPD->m_Type = PART_HAIR;
//del m_Type			else if(pItem->m_curValue == "��") pPD->m_Type = PART_FACE;
//del m_Type			else if(pItem->m_curValue == "��ü") pPD->m_Type = PART_UPPER;
//del m_Type			else if(pItem->m_curValue == "��ü") pPD->m_Type = PART_LOWER;
//del m_Type			else if(pItem->m_curValue == "��") pPD->m_Type = PART_HAND;
//del m_Type			else if(pItem->m_curValue == "��") pPD->m_Type = PART_FOOT;
...
//				}
		...
		if((void*)wParam == &m_LPCPlug && (pBase->Type() & OBJ_CHARACTER))
		{
			...
			if(nPlug >= 0 && nPlug < nPlugCount)
			{
				...
				{
					int nJI = atoi(pItem->m_curValue); // Joint Index �� ������ ����� ���ϵ���...
					int nJC = 0;
...
void CDlgBase::OnChangeEName() 
{
	...
	{
		CString str;
		GetDlgItemText(IDC_E_NAME, str); // �̸� �ٲٱ�..
		pBase->m_szName = str;
	}
}
