...
void CUIState::UpdateBonusPointAndButtons(int iBonusPointRemain) //    ??
{
	bool bEnable = false;
	...
	if(m_pBtn_Strength)		m_pBtn_Strength->SetVisible(bEnable); //  ..
...
void CUIState::UpdateRealmPoint(int iVal) //   10  
{
	if(NULL == m_pText_RealmPoint) return;
	m_pText_RealmPoint->SetStringAsInt(iVal/10); //   10  
}
...
void CUIState::MsgSendAblityPointChange(BYTE byType, short siValueDelta)
{
	...
	CAPISocket::MP_AddShort(byBuff, iOffset, siValueDelta); // 0x00 -  .. 0x01 -  ..
	...
	if (dwMsg == UIMSG_BUTTON_CLICK)					
	{
		...
		if(pSender == m_pBtn_PrevPage ||	pSender == m_pBtn_NextPage)
		{
			...
			else
			{
				...
				{
					this->MsgSend_MemberInfoAll(m_iPageCur); //   ..   ..
				}
				...
				{
					this->MsgSend_MemberInfoOnline(m_iPageCur); //   ..   ...
				}
			}
		}
		...
		{
			this->VisibleAppointButtons(true); //     ..
		}
...
bool CUIKnights::MsgRecv_MemberInfo(DataPack* pDataPack, int& iOffset)
{
	...
	for(int i=0;i<iKC;i++)
	{
		...
		this->MemberListAdd(szName, eDuty, eClass, iLevel, iConnected); // UI  ..
	}
	m_iPageCur = 1;
	sprintf(tmp,"%d", m_iPageCur);
	m_pText_Page->SetString(tmp);
...
void CUIKnights::ChangeUIByDuty(e_KnightsDuty eDuty) //   UI ..
{
...
/*
void CUIKnights::VisibleAppointButtons(bool bVisible) //    Interface
{
...
void CUIKnights::MsgSend_MemberJoinAdmit()
{
	...
	std::string szName = "???";
	CAPISocket::MP_AddShort(byBuff, iOffset, szName.size());	//  ..
...
bool CUIFriends::Load(HANDLE hFile)
{
	...
	std::string szFN = CGameProcedure::s_szAccount + "_" + CGameProcedure::s_szServer + ".txt"; //  _.txt  .
	FILE* pFile = fopen(szFN.c_str(), "r");
	if (pFile)
	{
		char szLine[256] = "";
		char* pszResult = fgets(szLine, 256, pFile); //  ..
		while(pszResult)
		{
			...
			pszResult = fgets(szLine, 256, pFile); //   ..
		}
		fclose(pFile);
...
//					iLinePerPage = (rc.bottom - rc.top) / dwH;
		...
		else if(pSender == m_pBtn_Add) // .
		{
			CPlayerOther* pUPC = CGameProcedure::s_pOPMgr->UPCGetByID(CGameProcedure::s_pPlayer->m_iIDTarget, false);
			if(pUPC)
			{
				if(this->MemberAdd(pUPC->IDString(), pUPC->IDNumber(), true, false)) //  ..
				{
					this->SaveListToTextFile(pUPC->IDString()); //   ..
		...
		else if(pSender == m_pBtn_Party) //  
		{
			...
			{
				std::string szMsg;
				if(pProcMain->MsgSend_PartyOrForceCreate(0, szID)) ::_LoadStringFromResource(IDS_PARTY_INVITE, szMsg); // 
				else  ::_LoadStringFromResource(IDS_PARTY_INVITE_FAILED, szMsg); //   
...
void CUIFriends::MsgSend_MemberInfo(bool bDisableInterval)
{
	...
	CAPISocket::MP_AddByte(&(buffers[0]), iOffset, N3_FRIEND_INFO); //  .. Send s1(), str1() | Receive s1(), str1(), s1(ID), b2(, )
...
void CUIFriends::MsgRecv_MemberInfo(DataPack* pDataPack, int& iOffset)
{
	...
	for(int i = 0; i < iFC; i++)
	{
		iLen = CAPISocket::Parse_GetShort(pDataPack->m_pData, iOffset); //  .. Send s1(), str1() | Receive s1(), str1(), s1(ID), b2(, )
...
bool CUIVarious::ReceiveMessage(CN3UIBase* pSender, DWORD dwMsg)
{
	if (dwMsg == UIMSG_BUTTON_CLICK)					
	{
		if(pSender == m_pBtn_Close)			this->Close(); // ..
		else if(pSender == m_pBtn_State)	this->UpdatePageButtons(m_pBtn_State);
//		else if(pSender == m_pBtn_Quest)	this->UpdatePageButtons(m_pBtn_Quest);		// ...
...
void CUIVarious::UpdatePageButtons(CN3UIButton* pButtonToActive)
{
	...
	if(pButtonToActive == m_pBtn_Friends && m_pPageFriends)
		m_pPageFriends->MsgSend_MemberInfo(false); //   ..
}
...
//	}
//	//  ...UI   ..
//	m_pPageKnights->ChangeUIByDuty(pInfoExt->eKnightsDuty); //   UI ..
}
...
//	sprintf(szVal, "%d", pInfoExt->iRank);			m_pText_Rank->SetString(szVal);
	...
	m_pPageState->UpdateBonusPointAndButtons(pInfoExt->iBonusPointRemain);  //    ??
	...
	//    ...
	m_pPageState->UpdateRealmPoint(pInfoExt->iRealmPoint); //   10  
