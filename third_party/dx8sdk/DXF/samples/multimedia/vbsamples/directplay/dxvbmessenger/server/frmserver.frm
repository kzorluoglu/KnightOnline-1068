...
Private Sub tmrLogoff_Timer()
    ...
    If mfLogoffTimer Then
        LogInfo "(tmrLogoff) Entering timer"
        NotifyFriendsImOffline msLogoffName
        LogInfo "(tmrLogoff) outta here:"
    End If
...
Private Sub tmrLogon_Timer()
    If mfLogonTimer Then
        mfLogonTimer = False
        LogInfo "(tmrLogin) Notify Friends:"
        NotifyFriendsImOnline msLogonName 'Tell everyone who has me marked as a friend that I'm online
        LogInfo "(tmrLogin) GetFriends:"
        GetFriendsOfMineOnline msLogonName 'Find out if any of my friends are online and tell me
        LogInfo "(tmrLogin) outta here:"
    End If
...
Private Sub DirectPlay8Event_Receive(dpnotify As DxVBLibA.DPNMSG_RECEIVE, fRejectMsg As Boolean)
    ...
    Case Msg_AddFriend 'They want to add a friend to their list
        LogInfo "(AddFriend) Message Received"
        ...
        If Not DoesUserExist(sUsername) Then
            ...
            LogInfo "(AddFriend) User does not exist"
    ...
    Case Msg_SendMessage 'They are trying to send a message to someone
        ...
        LogInfo "(Send Message) User/From/Chat"
        LogInfo "(Send Message) User " & sUsername
        LogInfo "(Send Message) From " & sFromMsg
        LogInfo "(Send Message) Chat " & sChatMsg
