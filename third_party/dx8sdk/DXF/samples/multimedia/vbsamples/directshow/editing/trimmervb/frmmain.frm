...
ErrLine:
            ...
            If ObjPtr(m_objMediaDet) > 0 Then Set m_objMediaDet = Nothing
            Exit Sub
...
' *
            ...
            'verify that media player's location is valid
            If File_Exists(MPLAYER2_INSTALL_LOCATION) Then
               'verify that the export location is valid
               If File_Exists(lblPlaybackFileName.Caption) Then
                  Shell (MPLAYER2_INSTALL_LOCATION & Space(1) & lblPlaybackFileName.Caption), vbNormalFocus
               Else: MsgBox "The file could not be found on the specified path: " & _
                        CStr(MPLAYER2_INSTALL_LOCATION & Space(1) & lblPlaybackFileName.Caption), vbExclamation + vbApplicationModal
               End If
            Else: MsgBox "Media player could not be found on the specified path: " & _
                        CStr(MPLAYER2_INSTALL_LOCATION), vbExclamation + vbApplicationModal
            End If
            Exit Sub
...
ErrLine:
            ...
            If ObjPtr(objMediaDet) > 0 Then Set objMediaDet = Nothing
            Exit Sub
...
Cleanup:
            ...
            If ObjPtr(objSmartRenderEngine) > 0 Then objSmartRenderEngine.ScrapIt
            ...
            If ObjPtr(objTimeline) > 0 Then Set objTimeline = Nothing
            If ObjPtr(objSourceObj) > 0 Then Set objSourceObj = Nothing
            If ObjPtr(objTrackObject) > 0 Then Set objTrackObject = Nothing
            If ObjPtr(objAudioGroupObj) > 0 Then Set objAudioGroupObj = Nothing
            If ObjPtr(objVideoGroupObject) > 0 Then Set objVideoGroupObject = Nothing
            'clean-up & dereference dexter timeline object(s)
            If ObjPtr(objTrack) > 0 Then Set objTrack = Nothing
            If ObjPtr(objSource) > 0 Then Set objSource = Nothing
            If ObjPtr(objAudioGroup) > 0 Then Set objAudioGroup = Nothing
            If ObjPtr(objVideoGroup) > 0 Then Set objVideoGroup = Nothing
            If ObjPtr(objAudioComposition) > 0 Then Set objAudioComposition = Nothing
            If ObjPtr(objVideoComposition) > 0 Then Set objVideoComposition = Nothing
...
ErrLine:
            ...
            Select Case Err.Number
                Case 5
                   MsgBox "Error creating file.  Verify that the start/stop times are valid before continuing.", vbExclamation + vbApplicationModal
