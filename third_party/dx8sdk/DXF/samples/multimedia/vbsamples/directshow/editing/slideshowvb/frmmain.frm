...
Begin VB.Form frmMain 
   ...
   Begin VB.Frame fraCommandFixture 
      ...
      Begin VB.Label lblInstructions 
         ...
         Left            =   150
         TabIndex        =   11
         ToolTipText     =   "Note: This Interface allows drag-and-drop editing.  Drag your video files into the poster frames to preview."
         Top             =   225
         Width           =   4140
      End
   End
...
ErrLine:
            ...
            If ObjPtr(gbl_objMediaControl) > 0 Then Set gbl_objMediaControl = Nothing
            If ObjPtr(gbl_objVideoWindow) > 0 Then Set gbl_objVideoWindow = Nothing
...
ErrLine:
            ...
            If ObjPtr(gbl_objTimeline) > 0 Then
               If ObjPtr(gbl_objRenderEngine) > 0 Then
...
ErrLine:
            ...
            bstrFileName = ShowCommonDlgSave(, "AVI", "*.avi|*.avi")
            ...
            If ObjPtr(objMediaEvent) > 0 Then Set objMediaEvent = Nothing
            If ObjPtr(objMediaPosition) > 0 Then Set objMediaPosition = Nothing
            If ObjPtr(objFilterGraphManager) > 0 Then Set objFilterGraphManager = Nothing
            If ObjPtr(objSmartRenderEngine) > 0 Then Set objSmartRenderEngine = Nothing
            Exit Sub
...
ErrLine:
            ...
            'delete the previous file if it exists
            bstrFileName = ShowCommonDlgSave(, "XTL", "*.xtl|*.xtl")
            If bstrFileName = vbNullString Then
               'enable the ui
               Call AppEnable(True, True, True)
               Exit Sub
            Else: If File_Exists(bstrFileName) Then _
...
' *
                        ...
                        If HasStreams(bstrCurrentFile) Then
                           If HasAudioStream(bstrCurrentFile) Then
                              ...
                              If GetGroupCount(objTimeline) > 0 Then
                                 For nCount2 = 0 To GetGroupCount(objTimeline)
                                    If ObjPtr(GroupFromTimeline(objTimeline, nCount2)) > 0 Then
            ...
            'return the timeline
            If ObjPtr(objTimeline) > 0 Then Set SpliceVideo = objTimeline
            
            'clean-up & dereference
            If ObjPtr(objTimeline) > 0 Then Set objTimeline = Nothing  ' AMTimeline
            If ObjPtr(objNewSource) > 0 Then Set objNewSource = Nothing  ' AMTimelineSrc
            If ObjPtr(objNewTrack) > 0 Then Set objNewTrack = Nothing  ' AMTimelineTrack
            If ObjPtr(objTransition) > 0 Then Set objTransition = Nothing ' AMTimelineTrans
            If ObjPtr(objAudioGroup) > 0 Then Set objAudioGroup = Nothing  ' AMTimelineGroup
            If ObjPtr(objVideoGroup) > 0 Then Set objVideoGroup = Nothing  ' AMTimelineGroup
            If ObjPtr(objTimelineTrackObject) > 0 Then Set objTimelineTrackObject = Nothing  ' AMTimelineObj
            If ObjPtr(objTimelineSourceObject) > 0 Then Set objTimelineSourceObject = Nothing  ' AMTimelineObj
            If ObjPtr(objTimelineAudioGroupObject) > 0 Then Set objTimelineAudioGroupObject = Nothing  ' AMTimelineObj
            If ObjPtr(objTimelineVideoGroupObject) > 0 Then Set objTimelineVideoGroupObject = Nothing  ' AMTimelineObj
            Exit Function
