...
' *
            ...
            If ObjPtr(objPin2) > 0 Then Set objPin2 = Nothing
            If ObjPtr(objPinInfo) > 0 Then Set objPinInfo = Nothing
            If ObjPtr(objFilterInfo) > 0 Then Set objFilterInfo = Nothing
            If ObjPtr(objPinCollection) > 0 Then Set objPinCollection = Nothing
            If ObjPtr(objlFilterCollection) > 0 Then Set objlFilterCollection = Nothing
            Exit Function
...
ErrLine:
            ...
            'clean-up & dereference
            If ObjPtr(objFilterInfo) > 0 Then Set objFilterInfo = Nothing
            If ObjPtr(objRegFilterInfo) > 0 Then Set objRegFilterInfo = Nothing
            If ObjPtr(objFileSinkFilterVB) > 0 Then Set objFileSinkFilterVB = Nothing
            If ObjPtr(objAVIMuxFilterInfo) > 0 Then Set objAVIMuxFilterInfo = Nothing
            If ObjPtr(objRegisteredFilters) > 0 Then Set objRegisteredFilters = Nothing
            Exit Sub
...
ErrLine:
            ...
            If ObjPtr(objTimeline) > 0 Then
               If ObjPtr(objRenderEngine) > 0 Then
