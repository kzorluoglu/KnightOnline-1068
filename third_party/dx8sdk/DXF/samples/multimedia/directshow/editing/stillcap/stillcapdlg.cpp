...
#define BITMAP_DATA_SIZE    (1024 * 400)  /* 400Kb */
...
public:
    ...
    STDMETHODIMP BufferCB( double dblSampleTime, BYTE * pBuffer, long lBufferSize )
    {
        ...
        // Don't try to write more data than the data buffer can store
        if (lBufferSize > BITMAP_DATA_SIZE)
            lBufferSize = BITMAP_DATA_SIZE;
    ...
    BOOL CopyBitmap( double dblSampleTime, BYTE * pBuffer, long lBufferSize )
    {
        ...
        wsprintf( m_szSnappedName, "%sStillCap%4.4ld.bmp", 
                  m_szCapDir, pOwner->m_nCapTimes );
        wsprintf( m_ShortName, "StillCap%4.4ld.bmp", 
                  pOwner->m_nCapTimes );
        ...
        if (pOwner->IsDlgButtonChecked(IDC_PLAYSOUND))
        {
            ...
            _tcscat(szSound, "\\media\\click.wav\0");
...
BOOL CStillCapDlg::OnInitDialog()
{
    ...
    // Initialize our callback scheme, saving the main application's
    // window handle and allocating a large block of memory known to
    // be large enough to hold the requested bitmap data.
    g_hwnd = GetSafeHwnd();
    cb.pBuffer = new BYTE[BITMAP_DATA_SIZE];
...
HRESULT CStillCapDlg::InitStillGraph( )
{
    ...
    if( !pCap )
    {
        Error( "No video capture device was detected on your system.\r\n\r\n"
    ...
    if( FAILED( hr ) )
    {
        Error( "Could not connect capture pin #0 to grabber.\r\n"
    ...
    pWindow->put_Owner( (long) hwndPreview );
    pWindow->put_Left( 0 );
    pWindow->put_Top( 0 );
...
HRESULT CStillCapDlg::InitCaptureGraph( TCHAR * pFilename )
{
    ...
    if( !pBuilder )
    {
        Error( "Could not create capture graph builder2" );
        return hr;
    }
    ...
    if( FAILED( hr ) )
    {
        Error( "Could not set filtergraph on graphbuilder2" );
        return hr;
    }
    ...
    if( FAILED( hr ) )
    {
        Error( "Could not create/hookup mux and writer" );
        return hr;
    }
    ...
    if( FAILED( hr ) )
    {
        Error( "Could not connect capture pin" );
        return hr;
    }
...
void CStillCapDlg::OnSnap() 
{
    ...
    else
    {
        ...
        char szFilename[MAX_PATH], szFile[MAX_PATH];
        wsprintf( szFilename, "%sStillCap%04d.avi", CapDir, m_nCapTimes );
        wsprintf( szFile, "StillCap%04d.avi", m_nCapTimes );
        ...
        if( m_nCapState == 0 )
        {
            ...
            SetDlgItemText( IDC_SNAP, "&Start Playback" );
            m_nCapState = 1;
        }
        else if( m_nCapState == 1 )
        {
            ...
            SetDlgItemText( IDC_SNAP, "&Start Capture" );
            m_nCapState = 0;
        }
    }
}
...
void CStillCapDlg::Error( char * pText )
{
    GetDlgItem( IDC_SNAP )->EnableWindow( FALSE );
    ::MessageBox( NULL, pText, "Error!", MB_OK | MB_TASKMODAL | MB_SETFOREGROUND );
}
...
void CStillCapDlg::OnCapstills() 
{
    ...
    SetDlgItemText( IDC_SNAP, "&Snap Still" );
    m_bCapStills = true;
...
void CStillCapDlg::OnButtonViewstill() 
{
    ...
    ShellExecute(this->GetSafeHwnd(), "open\0", mCB.m_szSnappedName, 
