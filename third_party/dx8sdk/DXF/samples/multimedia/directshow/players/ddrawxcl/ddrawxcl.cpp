...
//
// InitInstance(): Starts this instnce of the app (well, just creates the window).
//
...
BOOL FileSelect(HWND hWnd, VIDEO_TYPE eType)
{
    ...
    else
    {
        ...
        ofn.lpstrFilter = TEXT("AVI Files\0*.avi\0MPEG Files\0*.mpg\0All Files\0*.*\0") ;
    }
    ofn.nFilterIndex = 1 ;
    ...
    DWORD dwRes = CommDlgExtendedError() ;
    DbgLog((LOG_TRACE, 3, TEXT("GetOpenFileName() cancelled/failed (Error %lu)"), dwRes)) ;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_ACTIVATEAPP:
        ...
        if (! gbAppActive )  // loosing activation
        {
...
#if 0  // we don't need to paint the app window for the sample
    case WM_PAINT:
...
LRESULT CALLBACK KeyProc(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case VK_ESCAPE:
        MessageBeep(MB_ICONEXCLAMATION ) ; // just to warn that Esc has been hit
...
LRESULT StartPlay(HWND hWndApp)
{
    ...
    //
    guTimerID = SetTimer(hWndApp, TIMER_ID, TIMER_RATE, NULL) ;
    ASSERT(0 != guTimerID) ;
