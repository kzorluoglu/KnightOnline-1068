...
Private Sub cmdJoin_Click()
    ...
    If lstGames.ListIndex < 0 Then
        ...
        Exit Sub
    End If
    'We no longer need to Enum since we're connecting now
    If mlEnumAsync <> 0 Then cmdRefresh_Click
    ...
    HostAddr.BuildFromURL moHosts(lstGames.ItemData(lstGames.ListIndex)).Address
    If Not (moDPP Is Nothing) Then
        'Now we can join the selected session
        moDPP.Connect dpApp, HostAddr, moDPA, 0, ByVal 0&, 0
    ElseIf Not (moDPC Is Nothing) Then
        'Now we can join the selected session
        moDPC.Connect dpApp, HostAddr, moDPA, 0, ByVal 0&, 0
    End If
...
Private Sub cmdRefresh_Click()
    ...
    If mlSearch = StartSearch Then
        ...
        Dim dph As DirectPlay8Address
        
        Set dph = moDX.DirectPlayAddressCreate
        If Not (moDPP Is Nothing) Then
            dph.SetSP moDPP.GetServiceProvider(lstSP.ListIndex + 1).Guid
        ElseIf Not (moDPC Is Nothing) Then
            dph.SetSP moDPC.GetServiceProvider(lstSP.ListIndex + 1).Guid
        End If
        
        If Not (moDPP Is Nothing) Then
            mlEnumAsync = moDPP.EnumHosts(Desc, dph, moDPA, INFINITE, 0, INFINITE, DPNENUMHOSTS_OKTOQUERYFORADDRESSING, ByVal 0&, 0)
        ElseIf Not (moDPC Is Nothing) Then
            mlEnumAsync = moDPC.EnumHosts(Desc, dph, moDPA, INFINITE, 0, INFINITE, DPNENUMHOSTS_OKTOQUERYFORADDRESSING, ByVal 0&, 0)
        End If
...
Private Sub AddHostsToListBox(oHost As DPNMSG_ENUM_HOSTS_RESPONSE)
    ...
    If mlHostCount = -1 Then
        ...
        moHosts(0).Address = oHost.AddressSenderUrl
    ...
    Else
        ...
        If Not fFound Then 'We need to add this to the list
            ...
            moHosts(mlHostCount + 1).Address = oHost.AddressSenderUrl
            With oHost.ApplicationDescription
                lstGames.AddItem .SessionName & " - " & CStr(.lCurrentPlayers) & "/" & CStr(.lMaxPlayers) & " - Latency:" & CStr(oHost.lRoundTripLatencyMS) & " ms"
            End With
            'Save the last time this host was found
