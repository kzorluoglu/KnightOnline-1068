...
Private Sub cboUser_Click(Index As Integer)
    ...
    If DPlayEventsForm.IsHost Then
        If cboUser(Index).ListIndex = cboUser(Index).ListCount - 1 Then 'This slot is now closed
            ...
            If cboUser(Index).ItemData(0) > 0 Then 'There is already a user in this slot, kick them out
...
Private Sub DirectPlay8Event_CreatePlayer(ByVal lPlayerID As Long, fRejectMsg As Boolean)
    ...
    If DPlayEventsForm.IsHost Then 'If we are the host
        If (dpPeer.lPlayerFlags And DPNPLAYER_HOST) <> DPNPLAYER_HOST Then 'Don't notify ourselves
            ...
            cboUser(lCount).AddItem dpPeer.Name, 0
            cboUser(lCount).ItemData(0) = lPlayerID
            cboUser(lCount).ListIndex = 0
            ...
            AddDataToBuffer oBuf, lPlayerID, LenB(lPlayerID), lOffset
            'Send this message to everyone
            dpp.SendTo DPNID_ALL_PLAYERS_GROUP, oBuf, 0, DPNSEND_NOLOOPBACK Or DPNSEND_SYNC
            ...
            For lCount = 0 To 9
                If cboUser(lCount).ItemData(0) > 0 Then
                    If cboUser(lCount).ItemData(0) <> lPlayerID Then 'No need to pass this person twice
                        lMsg = MsgPutPlayerInSlot
                        lOffset = NewBuffer(oBuf)
                        AddDataToBuffer oBuf, lMsg, LenB(lMsg), lOffset
                        'Now add the slot number this player is in
                        ...
                        'Send this message to the joining player
                        dpp.SendTo lPlayerID, oBuf, 0, DPNSEND_NOLOOPBACK Or DPNSEND_SYNC
        ...
        Else
            ...
            cboUser(0).AddItem dpPeer.Name, 0
            cboUser(0).ItemData(0) = lPlayerID
...
Private Sub DirectPlay8Event_DestroyPlayer(ByVal lPlayerID As Long, ByVal lReason As Long, fRejectMsg As Boolean)
    ...
    For lCount = 0 To 9
        ...
        If cboUser(lCount).ItemData(0) = lPlayerID Then
            'Remove this person
...
Private Sub DirectPlay8Event_Receive(dpnotify As DxVBLibA.DPNMSG_RECEIVE, fRejectMsg As Boolean)
    ...
    Case MsgPutPlayerInSlot
        ...
        dpPeer = dpp.GetPeerInfo(lPlayerID)
        cboUser(lSlotID).AddItem dpPeer.Name, 0
        cboUser(lSlotID).ItemData(0) = lPlayerID
        cboUser(lSlotID).ListIndex = 0
        If (dpPeer.lPlayerFlags And DPNPLAYER_LOCAL) = DPNPLAYER_LOCAL Then chkReady(lSlotID).Enabled = True
...
Private Sub DirectPlay8Event_TerminateSession(dpnotify As DxVBLibA.DPNMSG_TERMINATE_SESSION, fRejectMsg As Boolean)
    ...
    If dpnotify.hResultCode = DPNERR_HOSTTERMINATEDSESSION Then
        ...
        Me.Hide
        dpp.UnRegisterMessageHandler 'We don't care about any other messages since we're quiting
    Else
