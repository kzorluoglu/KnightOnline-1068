...
STDMETHODIMP CDynamicSource::Stop(void)
{
    ...
    // if the pin is disconnected when CBaseFilter::Stop() is called.  If the
    // pin is connected, the thread will be destroyed in 
    // CDynamicSourceStream::Inactive().
    int nCurrentPin;
    ...
    for(nCurrentPin = 0; nCurrentPin < GetPinCount(); nCurrentPin++) {
        ...
        if (!pOutputPin->IsConnected() && pOutputPin->ThreadExists()) {
...
HRESULT CDynamicSourceStream::Inactive(void) {
    ...
    hr = CDynamicOutputPin::Inactive();  // call this first to Decommit the allocator
    if (FAILED(hr)) {
        return hr;
    }

    if (ThreadExists()) {
        hr = DestroySourceThread();
        if (FAILED(hr)) {
            return hr;
        }
    }
