...
int CConfigWnd::AddToList(const DIDEVICEINSTANCEW *lpdidi, BOOL bReset)
{
	...
	{
		i = m_CurSel;
		ClearElement(m_CurSel);  // If resetting, clear the current ELEMENT as we will populate it below.
	}
...
void CConfigWnd::ClearElement(int i)
{
	ELEMENT &e = GetElement(i);
	ClearElement(e);
}

void CConfigWnd::ClearElement(ELEMENT &e)
{
	if (e.pPage != NULL)
...
BOOL CConfigWnd::IsActionAssignedAnywhere(int nActionIndex)
{
	for (int i = 0; i < GetNumElements(); i++)
	{
		ELEMENT &e = GetElement(i);
		const LPDIACTIONFORMATW &lpAcFor = GetCurAcFor(e);
...
LRESULT CConfigWnd::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		...
		case WM_QUERYACTIONASSIGNEDANYWHERE:
			return IsActionAssignedAnywhere(int(lParam));
	}
...
{
	PostMessage(m_hWnd, WM_CFGUIRESET, 0, 0);
	return S_OK;
}

HRESULT CConfigWnd::QueryActionAssignedAnywhere(int i)
{
	return IsActionAssignedAnywhere(i) ? S_OK : S_FALSE;
}
...
LPDIACTIONFORMATW ELEMENT::GetAcFor(int nGenre, int nUser, BOOL bHwDefault)
{
	...
	// if we found it and its not null, return it
	if (bFound && lpAcFor)
		return lpAcFor;
